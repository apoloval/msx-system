...
; MSX SUBROM, Sanyo_PHC-35J version (v3.0)
...
J03B2:	LD	A,2CH
        LD	(XSAVE+1),A
        LD	A,26H
        LD	(YSAVE+1),A		; initialize lightpen variables
        LD	A,0D3H
        OUT	(0F7H),A
        LD	(AVCSAV),A
        XOR	A
...
J0423:	LD	A,C
        OUT	(0F5H),A
        CALL	C04E6			; check for JIS2 rom
        JR	NZ,J0430
        LD	HL,MODE
        SET	6,(HL)
J0430:	CALL	C1C85			; select bank 2 clockchip
        LD	B,00H
        CALL	C1C60			; read nibble from clockchip
        CP	0AH
        JR	Z,J043F
J043C:	CALL	C0497			; initialize bank 2 and 3 clockchip
J043F:	CALL	C1C85			; select bank 2 clockchip
        LD	B,03H
        CALL	C1C60			; read nibble from clockchip
        AND	01H
        PUSH	AF
        ...
        CP	32+1
        JP	NC,J043C
J045F:	CALL	C1C85			; select bank 2 clockchip
        LD	B,01H
        CALL	C1C60
        LD	D,A
        CALL	C1C60
...
J33F7:	LD	HL,XSAVE+1
        XOR	A
        BIT	7,(HL)			; lightpen interrupt ?
        RET	Z			; nope, quit
        RES	7,(HL)			; clear lightpen interrupt
        IN	A,(0BAH)
        AND	07H
        LD	D,A
        IN	A,(0B8H)
...
J3697:	PUSH	HL
        ...
        JR	Z,J36A5			; nope, �search� ram
        BIT	5,A			; ramdisk disabled ?
        JR	NZ,J36FA		; yep, illegal function call
        JR	J36FD

J36A5:	LD	HL,EXPTBL
        LD	BC,00400H		; four primairy slots, slotid 0
        XOR	A			; begin at primairy slot 0
J36AC:	AND	03H
        OR	(HL)			; if slot expanded, start at secundairy slot 0
J36AF:	PUSH	BC
        PUSH	HL
        LD	H,C
J36B2:	LD	L,10H
J36B4:	PUSH	AF
        ...
        POP	HL
        POP	HL
        LD	(SLTWRK+0),A		; slotid RAM for ramdisk
        JR	J36FD
...
J36E9:	POP	AF
        ...
        POP	BC
        AND	A			; slot expanded ?
        JP	P,J36F6			; nope, next primairy slot
        ADD	A,04H
        CP	90H			; done all secundairy slots ?
        JR	C,J36AF			; nope, next secundairy slot
J36F6:	INC	HL
        INC	A
        DJNZ	J36AC			; next primairy slot
J36FA:	JP	J274A			; illegal function call
