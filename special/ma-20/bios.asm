; BIOS.ASM

; MSX BIOS ROM, NEOS MA-20 version

; Source re-created by Z80DIS 2.2
; Z80DIS was written by Kenneth Gielow, Palo Alto, CA

; Code Copyrighted by ASCII and maybe others
; Source comments by Arjen Zeilemaker

; Sourcecode supplied for STUDY ONLY
; Recreation NOT permitted without authorisation of the copyrightholders


        .Z80
        ASEG
        ORG	0000H

        INCLUDE	MSX.INC


S.GRPPRT	EQU	0089h
S.MAPXYC	EQU	0091h
S.READC		EQU	0095h
S.SETC		EQU	009Dh
S.TRIGHT	EQU	00A1h
S.RIGHTC	EQU	00A5h
S.TLEFTC	EQU	00A9h
S.LEFTC		EQU	00ADh
S.TDOWNC	EQU	00B1h
S.DOWNC		EQU	00B5h
S.TUPC		EQU	00B9h
S.UPC		EQU	00BDh
S.SCANR		EQU	00C1h
S.SCANL		EQU	00C5h
S.CHGMOD	EQU	00D1h
S.INITXT	EQU	00D5h
S.INIT32	EQU	00D9h
S.INIGRP	EQU	00DDh
S.INIMLT	EQU	00E1h
S.SETTXT	EQU	00E5h
S.SETT32	EQU	00E9h
S.SETGRP	EQU	00EDh
S.SETMLT	EQU	00F1h
S.CLRSPR	EQU	00F5h
S.CLS		EQU	0115h
S.CLRTXT	EQU	0119h
S.DSPFNK	EQU	011Dh
S.DELLNO	EQU	0121h
S.INSLNO	EQU	0125h
S.WRTVDP	EQU	012Dh
S.PUTCHR	EQU	0139h
S.BEEP		EQU	017Dh
S.NEWPAD	EQU	01ADh
S.CHGMDP	EQU	01B5h

M409B	EQU	409BH	; Warm start msx basic
M4666	EQU	4666H	; CHRGTR routine in basicrom
M558C	EQU	558CH	; SYNCHR routine in basicrom
M5597	EQU	5597H	; GETYPR routine in basicrom
M60F6	EQU	60F6H	; headloop PRINT USING
M61F9	EQU	61F9H	; end of formatstring PRINT USING
M6246	EQU	6246H	; plussign if needed PRINT USING
M63E6	EQU	63E6H	; STOP statement in basicrom
M6C48	EQU	6C48H	; file i/o output
M7042	EQU	7042H	; PCMREC routine in basicrom
M71DB	EQU	71DBH	; PCMPLY routine in basicrom
M73B2	EQU	73B2H	; Device I/O error
M7B61	EQU	7B61H	; init code hardware part2
M7C76	EQU	7C76H	; init code msx basic
M7D75	EQU	7D75H	; extension ROM handler
M7E1A	EQU	7E1AH	; read word from extension ROM
M7FBE	EQU	7FBEH	; helper routine RDSLT in basicrom
M7FC4	EQU	7FC4H	; helper routine WRSLT in basicrom

D7405	EQU	7405H	; helper routine in diskrom

RDPRIM	EQU	0F380H
WRPRIM	EQU	0F385H
CLPRIM	EQU	0F38CH
CLPRM1	equ	0F398H
LINLEN	EQU	0F3B0H
CRTCNT	EQU	0F3B1H
T32COL	EQU	0F3BFH
GRPCGP	EQU	0F3CBH
CLIKSW	EQU	0F3DBH
CSRY	EQU	0F3DCH
CSRX	EQU	0F3DDH
CNSDFG	EQU	0F3DEH
RG0SAV	EQU	0F3DFH
RG1SAV	EQU	0F3E0H
STATFL	EQU	0F3E7H
TRGFLG	EQU	0F3E8H
FORCLR	EQU	0F3E9H
BAKCLR	EQU	0F3EAH
BDRCLR	EQU	0F3EBH
ATRBYT	EQU	0F3F2H
QUEUES	EQU	0F3F3H
SCNCNT	EQU	0F3F6H
REPCNT	EQU	0F3F7H
PUTPNT	EQU	0F3F8H
GETPNT	EQU	0F3FAH
LOW.	equ	0F406H
HIGH.	equ	0F408H
HEADER	equ	0F40AH
ASPCT1	EQU	0F40BH
ASPCT2	EQU	0F40DH
LPTPOS	EQU	0F415H
PRTFLG	EQU	0F416H
NTMSXP	EQU	0F417H
RAWPRT	EQU	0F418H
CURLIN	EQU	0F41CH
BUFMIN	EQU	0F55DH
BUF	EQU	0F55EH
TTYPOS	EQU	0F661H
AUTFLG	EQU	0F6AAH
SAVSTK	EQU	0F6B1H
PTRFIL	EQU	0F864H
FILNAM	EQU	0F866H
FNKSTR	EQU	0F87FH
CGPNT	EQU	0F91FH
NAMBAS	EQU	0F922H
CGPBAS	EQU	0F924H
PATBAS	EQU	0F926H
ATRBAS	EQU	0F928H
CLOC	EQU	0F92AH
CMASK	EQU	0F92CH
CSAVEA	EQU	0F942H
CSAVEM	EQU	0F944H
QUEBAK	EQU	0F971H
VOICAQ	EQU	0F975H
ACPAGE	EQU	0FAF6H
EXBRSA	EQU	0FAF8H
CHRCNT	EQU	0FAF9H
MODE	EQU	0FAFCH
XSAVE	EQU	0FAFEH
LOGOPR	EQU	0FB02H
HOKVLD	EQU	0FB20H
VOICEN	EQU	0FB38H
QUEUEN	EQU	0FB3EH
MUSICF	EQU	0FB3FH
PLYCNT	EQU	0FB40H
VCBA	EQU	0FB41H
VCBB	EQU	0FB66H
VCBC	EQU	0FB8BH
FNKFLG	EQU	0FBCEH
TRPTBL	EQU	0FC4CH
ENSTOP	EQU	0FBB0H
BASROM	EQU	0FBB1H
LINTTB	EQU	0FBB2H
FSTPOS	EQU	0FBCAH
CURSAV	EQU	0FBCCH
FNKSWI	EQU	0FBCDH
ONGSBF	EQU	0FBD8H
CLIKFL	EQU	0FBD9H
OLDKEY	EQU	0FBDAH
NEWKEY	EQU	0FBE5H
KEYBUF	EQU	0FBF0H
LINWRK	EQU	0FC18H
PATWRK	EQU	0FC40H
INTFLG	EQU	0FC9BH
PADY	EQU	0FC9CH
PADX	EQU	0FC9DH
JIFFY	EQU	0FC9EH
INTVAL	EQU	0FCA0H
INTCNT	EQU	0FCA2H
LOWLIM	equ	0FCA4H
WINWID	equ	0FCA5H
GRPHED	EQU	0FCA6H
ESCCNT	EQU	0FCA7H
INSFLG	EQU	0FCA8H
CSRSW	EQU	0FCA9H
CSTYLE	EQU	0FCAAH
CAPST	EQU	0FCABH
KANAST	EQU	0FCACH
KANAMD	EQU	0FCADH
SCRMOD	EQU	0FCAFH
OLDSCR	EQU	0FCB0H
CASPRV	EQU	0FCB1H
BDRATR	EQU	0FCB2H
GRPACX	EQU	0FCB7H
GRPACY	EQU	0FCB9H
EXPTBL	EQU	0FCC1H
SLTTBL	EQU	0FCC5H
SLTATR	EQU	0FCC9H

H.KEYI	EQU	0FD9AH
H.TIMI	EQU	0FD9FH
H.CHPU	EQU	0FDA4H
H.DSPC	EQU	0FDA9H
H.ERAC	EQU	0FDAEH
H.DSPF	EQU	0FDB3H
H.ERAF	EQU	0FDB8H
H.TOTE	EQU	0FDBDH
H.CHGE	EQU	0FDC2H
H.INIP	EQU	0FDC7H
H.KEYC	EQU	0FDCCH
H.KEYA	EQU	0FDD1H
H.NMI	EQU	0FDD6H
H.PINL	EQU	0FDDBH
H.QINL	EQU	0FDE0H
H.INLI	EQU	0FDE5H
H.ISFL	EQU	0FEDFH
H.OUTD	EQU	0FEE4H
H.OKNO	EQU	0FF75H
H.MDIN	EQU	0FF75H
H.MDTM	EQU	0FF93H
H.PHYD	EQU	0FFA7H
H.FORM	EQU	0FFACH
H.LPTO	EQU	0FFB6H
H.LPTS	EQU	0FFBBH
EXTBIO	EQU	0FFCAH
RG8SAV	EQU	0FFE7H

D.FFFF	EQU	0FFFFH	; secundairy slotregister

        PUBLIC	J0416
        PUBLIC	J044C
        EXTRN	J7DF6

CHKRAM:	DI
        JP	J0416

        DEFW	D1BBF			; address character pattern
        DEFB	088H			; vdp dataregister i/o address for read
        DEFB	088H			; vdp dataregister i/o address for write


SYNCHR:	JP	J2683

        DEFS	0000CH-$

RDSLT:	JP	C01F5

        DEFS	00010H-$

CHRGTR:	JP	J2686

        DEFS	00014H-$

WRSLT:	JP	C0225

        DEFS	00018H-$

OUTDO:	JP	J1B45

        DEFS	0001CH-$

CALSLT:	JP	C0217

        DEFS	00020H-$

DCOMPR:	JP	J0C04

        DEFS	00024H-$

ENASLT:	JP	C0333

        DEFS	00028H-$

GETYPR:	JP	J2689

IDBYT0:
        IF	INTHZ EQ 60
        DEFB	CHRGEN+16*DATFMT
        ELSE
        DEFB	CHRGEN+16*DATFMT+128
        ENDIF
IDBYT1:	DEFB	KEYTYP+16*BASVER
IDBYT2:	DEFB	MSXVER			; MSX version 1 = MSX2
IDBYT3:	DEFB	0

        DEFS	00030H-$

CALLF:	JP	J02C6

        DEFS	00034H-$

;	The next bytes are used by the diskrom, to initialize the double byte header char
;	table (0F30FH). I have not seen a MSX with anything other than four zero's, meaning
;	no double byte chars.

D0034:	DEFB	0,0			; double byte header char range 1
        DEFB	0,0			; double byte header char range 2

KEYINT:	JP	J0C3C
INITIO:	JP	J0592
INIFNK:	JP	J13A0
DISSCR:	JP	J060A
ENASCR:	JP	J0603
WRTVDP:	JP	J0612
RDVRM:	JP	C07E1
WRTVRM:	JP	C07D7
SETRD:	JP	C0808
SETWRT:	JP	C07F4
FILVRM:	JP	J083B
LDIRMV:	JP	C073D
LDIRVM:	JP	C0780
CHGMOD:	JP	J08B1
CHGCLR:	JP	J081A

        DEFS	00066H-$		; align to Z80 NMI entry at 0066H

NMI:	JP	J139B
CLRSPR:	JP	J067F
INITXT:	JP	J063E
INIT32:	JP	J0647
INIGRP:	JP	J064F
INIMLT:	JP	J0657
SETTXT:	JP	J065F
SETT32:	JP	J0667
SETGRP:	JP	J066F
SETMLT:	JP	J0677
CALPAT:	JP	J06EC
CALATR:	JP	J0701
GSPSIZ:	JP	C070C
GRPPRT:	JP	J150E
GICINI:	JP	C05B2
WRTPSG:	JP	C1102
RDPSG:	JP	C110E
STRTMS:	JP	J11BA
CHSNS:	JP	C0D6A
CHGET:	JP	C10CB
CHPUT:	JP	J0919
LPTOUT:	JP	C08BA
LPTSTT:	JP	C08E1
CNVCHR:	JP	C08FA
PINLIN:	JP	J23BF
INLIN:	JP	J23D5
QINLIN:	JP	J23CC
BREAKX:	JP	C0564
ISCNTC:	JP	C04F0
CKCNTC:	JP	J10F9
BEEP:	JP	C1113
CLS:	JP	J0897
POSIT:	JP	C08EB
FNKSB:	JP	J0B3A
ERAFNK:	JP	J0B29
DSPFNK:	JP	C0B3F
TOTEXT:	JP	C089E
GTSTCK:	JP	J11E4
GTTRIG:	JP	J1249
GTPAD:	JP	J12A2
GTPDL:	JP	J1269
TAPION:	JP	J1A63
TAPIN:	JP	J1ABC
TAPIOF:	JP	J19E9
TAPOON:	JP	J19F1
TAPOUT:	JP	J1A19
TAPOOF:	JP	J19DD
STMOTR:	JP	J1387
LFTQ:	JP	J14E9
PUTQ:	JP	J1490
RIGHTC:	JP	C1756
LEFTC:	JP	J1790
UPC:	JP	J181F
TUPC:	JP	J17F6
DOWNC:	JP	J17DC
TDOWNC:	JP	C17B4
SCALXY:	JP	C15A5
MAPXYC:	JP	C1604
FETCHC:	JP	C1651
STOREC:	JP	C1658
SETATR:	JP	J1692
READC:	JP	C165F
SETC:	JP	C16C3
NSETCX:	JP	C1843
GTASPC:	JP	J1901
PNTINI:	JP	J1909
SCANR:	JP	J191E
SCANL:	JP	J1994
CHGCAP:	JP	K.BCAP
CHGSND:	JP	K.BSND
RSLREG:	JP	J140E
WSLREG:	JP	J1411
RDVDP:	JP	J140B
SNSMAT:	JP	J0BEC
PHYDIO:	JP	J1414
FORMAT:	JP	J1418
ISFLIO:	JP	C0BF9
OUTDLP:	JP	C1B63
GETVCP:	JP	C0C0A
GETVC2:	JP	J0C0E
KILBUF:	JP	C055D
CALBAS:	JP	C02BF
SUBROM:	JP	J0295
EXTROM:	JP	C029B
CHKSLZ:	JP	J03AC
CHKNEW:	JP	C06A9
EOL:	JP	C0B05
BIGFIL:	JP	C0838
NSETRD:	JP	C06C5
NSTWRT:	JP	C06B3
NRDVRM:	JP	J07E9
NWRVRM:	JP	J0769

        DEFS	0017AH-$

;	  Subroutine replace slotid 0 or 0-0 with slotid of MA-20 mainrom (for page 0 and 1)
;	     Inputs  ________________________
;	     Outputs ________________________

C017A:	BIT	7,H			; page 0 or 1 ?
        JR	NZ,J018D		; nope, use normal methode
        BIT	7,A			; slot expanded ?
        JR	NZ,J0184		; yep,
        AND	03H			; clear unused bits
J0184:	LD	B,A
        AND	0FH
        LD	A,B			; slot 0 or slot 0-0 ?
        JR	NZ,J018D		; nope, use normal methode
        LD	A,(EXPTBL+0)		; use slot id of MA-20 mainrom
J018D:	JP	C0353

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C0190:	PUSH	BC
        LD	A,8
        CALL	SNSMAT
        BIT	3,A			; DEL key pressed ?
        POP	BC
        JP	NZ,M7E1A		; nope, read word from extension ROM
        LD	A,(EXPTBL+0)
        XOR	C
        AND	03H			; same primairy slot as MA-20 mainrom ?
        JP	Z,M7E1A			; nope, read word from extension ROM
        LD	D,A			; make sure DE <> 04241H
        INC	HL
        INC	HL
        RET

        DEFS	001B6H-$

J01B6:	POP	AF
        CALL	C143E
        LD	H,A
        IN	A,(0A8H)
        LD	B,A
        AND	3FH
        OR	D
        OUT	(0A8H),A
        LD	D,A
        LD	A,(D.FFFF)
        CPL
        LD	C,A
        AND	0FCH
        OR	H
        LD	E,A
        LD	(D.FFFF),A

; from this point code continues to execute in subrom

        LD	A,L
        OUT	(0A8H),A
        PUSH	HL
        CALL	C04D0
        LD	(HL),E
        PUSH	HL
        PUSH	BC
        EXX
        EX	AF,AF'
        CALL	CLPRM1
        DI
        EX	AF,AF'
        EXX
        POP	BC
        POP	HL
        POP	AF
        OUT	(0A8H),A
        LD	A,C
        LD	(D.FFFF),A

; from this point code returns executing from the subrom

        LD	A,B
        OUT	(0A8H),A
        LD	(HL),C
        EX	AF,AF'
        EXX
        RET

        DEFS	01F5H-$

;	Subroutine	RDSLT
;	Inputs		________________________
;	Outputs		________________________


C01F5:	CALL	C017A
        JP	M,J0205
        IN	A,(0A8H)
        LD	D,A
        AND	C
        OR	B
        CALL	RDPRIM
        LD	A,E
        RET

;	Subroutine	RDSLT for expanded slot
;	Inputs		________________________
;	Outputs		________________________


J0205:	CALL	C028C
        JR	NZ,J021A
        PUSH	HL
        CALL	C0255
        EX	(SP),HL
        CALL	M7FBE
        JR	J0244

        DEFS	0217H-$			; align to 0217H

;	Subroutine	CALSLT
;	Inputs		________________________
;	Outputs		________________________
;	Remark		Just a jump at this adres to have same entry point as MSX1

C0217:	JP	J02D8


;	Subroutine	RDSLT for expanded slot, not slot 0 page 0
;	Inputs		________________________
;	Outputs		________________________


J021A:	PUSH	HL
        CALL	C0378
        EX	(SP),HL
        PUSH	BC
        CALL	C01F5
        JR	J0279

;	Subroutine	WRSLT
;	Inputs		________________________
;	Outputs		________________________


C0225:	PUSH	DE
        CALL	C017A
        JP	M,J0235
        POP	DE
        IN	A,(0A8H)
        LD	D,A
        AND	C
        OR	B
        JP	WRPRIM

;	Subroutine	WRSLT for expanded slot
;	Inputs		________________________
;	Outputs		________________________


J0235:	CALL	C028C
        JP	NZ,J026E
        POP	DE
        PUSH	HL
        CALL	C0255
        EX	(SP),HL
        CALL	M7FC4
J0244:	EX	(SP),HL
        PUSH	AF
        CALL	C1452
        OUT	(0A8H),A
        LD	A,H
        LD	(D.FFFF),A
        LD	A,L
        OUT	(0A8H),A
        POP	AF
        POP	HL
        RET

;	Subroutine	basicrom on page 1
;	Inputs		________________________
;	Outputs		________________________


C0255:	PUSH	AF
        IN	A,(0A8H)
        CALL	C1453
        OUT	(0A8H),A
        LD	A,(D.FFFF)
        CPL
        LD	H,A
        AND	0F3H
        LD	(D.FFFF),A
        JP	J145D
J026A:	OUT	(0A8H),A
        POP	AF
        RET

;	Subroutine	WRSLT for expanded slot, not slot 0 page 0
;	Inputs		________________________
;	Outputs		________________________


J026E:	EX	(SP),HL
        PUSH	HL
        CALL	C0378
        POP	DE
        EX	(SP),HL
        PUSH	BC
        CALL	C0225
J0279:	POP	BC
        EX	(SP),HL
        PUSH	AF
        LD	A,B
        AND	3FH
        OR	C
        OUT	(0A8H),A
        LD	A,L
        LD	(D.FFFF),A
        LD	A,B
        OUT	(0A8H),A
        POP	AF
        POP	HL
        RET

;	Subroutine	check if primary slot 0,page 0
;	Inputs		________________________
;	Outputs		Zx if primary slot 0, page 0

C028C:	CALL	C1448
        RET	NZ
        LD	A,E
        CP	03H
        LD	A,B
        RET

;	Subroutine	SUBROM
;	Inputs		________________________
;	Outputs		________________________


J0295:	CALL	C029B
        POP	IX
        RET

;	Subroutine	EXTROM
;	Inputs		________________________
;	Outputs		________________________


C029B:	EXX
        EX	AF,AF'
        PUSH	HL
        PUSH	DE
        PUSH	BC
        PUSH	AF
        LD	A,I
        PUSH	AF
        EXX
        EX	AF,AF'
        PUSH	IY
        LD	IY,(EXBRSA-1)
        CALL	C0217
        POP	IY
        EX	AF,AF'
        EXX
        POP	AF
        JP	PO,J02B8
        EI
J02B8:	POP	AF
        POP	BC
        POP	DE
        POP	HL
        EXX
        EX	AF,AF'
        RET

;	Subroutine	CALBAS
;	Inputs		________________________
;	Outputs		________________________


C02BF:	LD	IY,(EXPTBL+0-1)
        JP	C0217

;	Subroutine	CALLF
;	Inputs		________________________
;	Outputs		________________________


J02C6:	EX	(SP),HL
        PUSH	AF
        PUSH	DE
        LD	A,(HL)
        PUSH	AF
        POP	IY
        INC	HL
        LD	E,(HL)
        INC	HL
        LD	D,(HL)
        INC	HL
        PUSH	DE
        POP	IX
        POP	DE
        POP	AF
        EX	(SP),HL

;	Subroutine	CALSLT
;	Inputs		________________________
;	Outputs		________________________


J02D8:	EXX
        EX	AF,AF'
        PUSH	IY
        POP	AF
        PUSH	IX
        POP	HL
        CALL	C017A
        JP	M,J02EF
        IN	A,(0A8H)
        PUSH	AF
        AND	C
        OR	B
        EXX
        JP	CLPRIM

;	Subroutine	CALSLT for expanded slot
;	Inputs		________________________
;	Outputs		________________________


J02EF:	PUSH	AF
        PUSH	HL
        LD	HL,EXPTBL+0
        XOR	(HL)			; slotid of the MA-20 mainrom ?
        AND	E			; ?
        POP	HL
        JP	Z,J142E
J02FA:	POP	AF
        CALL	C0378
        PUSH	AF
        POP	IY
        PUSH	HL
        PUSH	BC
        LD	C,A
        LD	B,00H
        LD	A,L
        AND	H
        OR	D
        LD	HL,SLTTBL
        ADD	HL,BC
        LD	(HL),A
        PUSH	HL
        EX	AF,AF'
        EXX
        CALL	C0217
        EXX
        EX	AF,AF'
        POP	HL
        POP	BC
        POP	DE
        LD	A,I
        PUSH	AF
        LD	A,B
        AND	3FH
        OR	C
        DI
        OUT	(0A8H),A
        LD	A,E
        LD	(D.FFFF),A
        LD	A,B
        OUT	(0A8H),A
        LD	(HL),E
        POP	AF
        JP	PO,J0330
        EI
J0330:	EX	AF,AF'
        EXX
        RET

;	Subroutine	ENASLT
;	Inputs		________________________
;	Outputs		________________________


C0333:	CALL	C017A
        JP	M,J0340
        IN	A,(0A8H)
        AND	C
        OR	B
        OUT	(0A8H),A
        RET

;	Subroutine	ENASLT for expanded slot
;	Inputs		________________________
;	Outputs		________________________


J0340:	PUSH	HL
        CALL	C0378
        LD	C,A
        LD	B,00H
        LD	A,L
        AND	H
        OR	D
        LD	HL,SLTTBL
        ADD	HL,BC
        LD	(HL),A
        POP	HL
        LD	A,C
        JR	C0333

;	Subroutine	calculate masks
;	Inputs		A=slotid, HL=adres
;	Outputs		P set if expanded slot, A=slotid, D=PPPPPPPP, E=page select mask
;	             B=primairy slot or mask, C=page clear mask

C0353:	DI
        PUSH	AF
        LD	A,H
        RLCA
        RLCA
        AND	03H
        LD	E,A
        LD	A,0C0H
J035D:	RLCA
        RLCA
        DEC	E
        JP	P,J035D
        LD	E,A
        CPL
        LD	C,A
        POP	AF
        PUSH	AF
        AND	03H	; 3
        INC	A
        LD	B,A
        LD	A,0ABH
J036E:	ADD	A,55H
        DJNZ	J036E
        LD	D,A
        AND	E
        LD	B,A
        POP	AF
        AND	A
        RET

;	Subroutine	Adjust secundairy slotregister
;	Inputs		A=slotid, D=, E=
;	Outputs		A=slotid primairy slot

C0378:	PUSH	AF
        LD	A,D
        AND	0C0H
        LD	C,A
        POP	AF
        PUSH	AF
        LD	D,A
        IN	A,(0A8H)
        LD	B,A
        AND	3FH
        OR	C
        OUT	(0A8H),A
        LD	A,D
        RRCA
        RRCA
        AND	03H	; 3
        LD	D,A
        LD	A,0ABH
J0390:	ADD	A,55H
        DEC	D
        JP	P,J0390
        AND	E
        LD	D,A
        LD	A,E
        CPL
        LD	H,A
        LD	A,(D.FFFF)
        CPL
        LD	L,A
        AND	H
        OR	D
        LD	(D.FFFF),A
        LD	A,B
        OUT	(0A8H),A
        POP	AF
        AND	03H
        RET

;	Subroutine	CHKSLZ
;	Inputs		________________________
;	Outputs		________________________

J03AC:	DI
        CALL	C03DB			; search subrom
        LD	HL,2			; init entry subrom
        CALL	M7E1A			; read word from extension ROM
        PUSH	HL
        PUSH	BC
        PUSH	DE
        POP	IX
        LD	A,C
        PUSH	AF
        POP	IY
        CALL	NZ,C0217		; CALSLT
        POP	BC
        POP	HL
        CALL	M7E1A			; read word from extension ROM
        ADD	A,0FFH
        RR	B
        CALL	M7E1A			; read word from extension ROM
        ADD	A,0FFH
        RR	B
        SRL	B
        CALL	C04D8
        LD	(HL),B
        JP	M7D75

;	  Subroutine search subrom
;	     Inputs  ________________________
;	     Outputs ________________________

C03DB:	LD	C,0			; slot 0
        LD	DE,EXPTBL
J03E0:	LD	A,(DE)
        AND	80H
        OR	C
        LD	C,A
        PUSH	DE
J03E6:	LD	HL,0			; header page 0
        CALL	M7E1A			; read word from extension ROM
        LD	HL,04443H
        RST	20H			; subrom header ?
        LD	A,C
        JP	Z,J087F			; yep, quit search
        AND	A			; slot expanded ?
        JP	P,J0402			; nope, next primairy slot
        ADD	A,04H
        LD	C,A
        CP	90H
        JR	C,J03E6
        AND	03H
        LD	C,A
J0402:	POP	DE
        INC	DE
        INC	C
        LD	A,C
        CP	04H	; 4
        JR	C,J03E0
        SCF
        RET

J040C:	LD	HL,SLTATR+(0 AND 3)*16+(0 AND 12)+0
        LD	A,(HL)
        LD	(EXPTBL+0),A
        LD	(HL),0
        RET

; orginal init MSX2 and above

J0416:	XOR	A
        OUT	(0FFH),A
        INC	A
        OUT	(0FEH),A
        INC	A
        OUT	(0FDH),A
        INC	A
        OUT	(0FCH),A		; initialize memory mapper
        LD	A,08H
        OUT	(0BBH),A
        LD	A,50H
        OUT	(0AAH),A
        CALL	C03DB			; search subrom
        JP	C,J040C			; not found, restore orginal EXPTBL flag for slot 0 and quit
        PUSH	AF
        LD	A,(RG1SAV)
        AND	0DFH			; clear IE bit
        OUT	(99H),A
        POP	IY
        LD	IX,D7405		; routine in diskrom, which is in the same slot as the subrom
        LD	A,81H
        OUT	(99H),A			; disable interrupts from MSX1 VDP (TMS9918)
        EX	(SP),HL
        EX	(SP),HL
        IN	A,(99H)			; clear pending interrupts MSX1 VDP
        CALL	C0217			; CALSLT (change system RAM page 3 if RAM in same slot as MA-20 found)
        JP	M7B61

J044C:	DI
        LD	C,00H
        LD	DE,EXPTBL
        LD	HL,SLTATR
J0455:	LD	A,(DE)
        AND	80H
        OR	C
        LD	C,A
        PUSH	DE
J045B:	INC	HL
        PUSH	HL
        LD	HL,04000H
J0460:	CALL	C0190			; read word from extension ROM (with skip feature)
        PUSH	HL
        LD	HL,04241H
        RST	20H
        POP	HL			; extension ROM header ?
        LD	B,0			; clear extension flags
        JR	NZ,J0497		; nope,
        CALL	M7E1A			; read word from extension ROM
        PUSH	HL
        PUSH	BC
        PUSH	DE
        POP	IX
        LD	A,C
        PUSH	AF
        POP	IY
        CALL	NZ,C0217		; CALSLT
        POP	BC
        POP	HL
        CALL	M7E1A			; read word from extension ROM
        ADD	A,0FFH
        RR	B
        CALL	M7E1A			; read word from extension ROM
        ADD	A,0FFH
        RR	B
        CALL	M7E1A			; read word from extension ROM
        ADD	A,0FFH
        RR	B
        LD	DE,-8
        ADD	HL,DE
J0497:	EX	(SP),HL
        LD	(HL),B
        INC	HL
        EX	(SP),HL
        LD	DE,4000H-2
        ADD	HL,DE
        LD	A,H
        CP	0C0H
        JR	C,J0460
        POP	HL
        INC	HL
        LD	A,C
        AND	A
        LD	DE,12
        JP	P,J04B9
        ADD	A,04H
        LD	C,A
        CP	90H
        JR	C,J045B
        AND	03H
        LD	C,A
        DEFB	03EH
J04B9:	ADD	HL,DE
        POP	DE
        INC	DE
        INC	C
        LD	A,C
        CP	04H
        JR	C,J0455
        LD	HL,SLTATR
        LD	B,40H
J04C7:	LD	A,(HL)
        ADD	A,A
        JP	C,J7DF6
        INC	HL
        DJNZ	J04C7
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C04D0:	AND	03H
        LD	HL,SLTTBL
        ADD	A,L
        LD	L,A
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C04D8:	LD	A,C
        ADD	A,A
        RLCA
        RLCA
        RLCA
        OR	C
        AND	3CH	; "<"
        ADD	A,0C9H
        LD	L,A
        LD	A,00H
        ADC	A,0FCH
        LD	H,A
        RET

        DEFS	04F0H-$,0

;	Subroutine	ISCNTC
;	Inputs		________________________
;	Outputs		________________________

C04F0:	LD	A,(BASROM)
        AND	A
        RET	NZ
        PUSH	HL
        LD	HL,INTFLG
        DI
        LD	A,(HL)
        EI
        LD	(HL),00H
        POP	HL
        AND	A
        RET	Z
        CP	03H	; 3
        JR	Z,J0521
        PUSH	HL
        PUSH	DE
        PUSH	BC
        CALL	C0A37
        LD	HL,INTFLG
J050E:	DI
        LD	A,(HL)
        EI
        LD	(HL),00H
        AND	A
        JR	Z,J050E
        PUSH	AF
        CALL	C0A84
        POP	AF
        POP	BC
        POP	DE
        POP	HL
        CP	03H	; 3
        RET	NZ
J0521:	PUSH	HL
        CALL	C055D
        CALL	C0549
        JR	NC,J0534
        LD	HL,TRPTBL+10*3
        DI
        CALL	C0EF1
        EI
        POP	HL
        RET

J0534:	CALL	C089E
        LD	A,(EXPTBL+0)
        LD	H,40H	; "@"
        CALL	C0333
        POP	HL
        XOR	A
        LD	SP,(SAVSTK)
        PUSH	BC
        JP	M63E6

;	Subroutine	check if ON STOP valid
;	Inputs		________________________
;	Outputs		________________________


C0549:	LD	A,(TRPTBL+10*3+0)
        RRCA
        RET	NC
        LD	HL,(TRPTBL+10*3+1)
        LD	A,H
        OR	L
        RET	Z
        LD	HL,(CURLIN)
        INC	HL
        LD	A,H
        OR	L
        RET	Z
        SCF
        RET

;	Subroutine	KILBUF (clears keyboardbuffer)
;	Inputs		________________________
;	Outputs		________________________


C055D:	LD	HL,(PUTPNT)
        LD	(GETPNT),HL
        RET

;	Subroutine	BREAKX
;	Inputs		________________________
;	Outputs		________________________


C0564:	IN	A,(0AAH)
        AND	0F0H
        OR	07H	; 7
        OUT	(0AAH),A
        IN	A,(0A9H)
        AND	10H	; 16
        RET	NZ
        IN	A,(0AAH)
        DEC	A
        OUT	(0AAH),A
        IN	A,(0A9H)
        AND	02H	; 2
        RET	NZ
        PUSH	HL
        LD	HL,(PUTPNT)
        LD	(GETPNT),HL
        POP	HL
        LD	A,(OLDKEY+7)
        AND	0EFH
        LD	(OLDKEY+7),A

        IF INTHZ EQ 60
        LD	A,14H
        ELSE
        LD	A,20H
        ENDIF

        LD	(REPCNT),A
        SCF
        RET

;	Subroutine	INITIO
;	Inputs		________________________
;	Outputs		________________________


J0592:	LD	A,07H	; 7
        LD	E,80H
        CALL	C1102
        LD	A,0FH	; 15
        LD	E,0CFH
        CALL	C1102
        LD	A,0BH	; 11
        LD	E,A
        CALL	C1102
        CALL	C110C
        AND	40H	; "@"
        LD	(KANAMD),A
        LD	A,0FFH
        OUT	(90H),A

;	Subroutine	GICINI
;	Inputs		________________________
;	Outputs		________________________


C05B2:	PUSH	HL
        PUSH	DE
        PUSH	BC
        PUSH	AF
        LD	HL,MUSICF
        LD	B,71H	; "q"
        XOR	A
J05BC:	LD	(HL),A
        INC	HL
        DJNZ	J05BC
        LD	DE,VOICAQ
        LD	B,7FH
        LD	HL,0080H
J05C8:	PUSH	HL
        PUSH	DE
        PUSH	BC
        PUSH	AF
        CALL	C14D8
        POP	AF
        ADD	A,08H	; 8
        LD	E,00H
        CALL	C1102
        SUB	08H	; 8
        PUSH	AF
        LD	L,0FH	; 15
        CALL	C0C11
        EX	DE,HL
        LD	HL,I05FD
        LD	BC,6
        LDIR
        POP	AF
        POP	BC
        POP	HL
        POP	DE
        ADD	HL,DE
        EX	DE,HL
        INC	A
        CP	03H	; 3
        JR	C,J05C8
        LD	A,07H	; 7
        LD	E,0B8H
        CALL	C1102
        JP	J0937

I05FD:	DEFB	004H,004H,078H,088H,0FFH,000H

;	Subroutine	ENASCR
;	Inputs		________________________
;	Outputs		________________________


J0603:	LD	A,(RG1SAV)
        OR	40H	; "@"
        JR	J060F

;	Subroutine	DISSCR
;	Inputs		________________________
;	Outputs		________________________


J060A:	LD	A,(RG1SAV)
        AND	0BFH
J060F:	LD	B,A
        LD	C,01H	; 1

;	Subroutine	WRTVDP
;	Inputs		________________________
;	Outputs		________________________


J0612:	LD	A,C
        AND	A
        JR	NZ,J061D
        LD	A,(RG0SAV)
        XOR	B
        RRCA
        JR	NC,J0629		; external vdp input bit unchanged, old way
                                        ; otherwise A>=080H and due next code handled by the subrom
J061D:	CP	08H	; 8
        JR	C,J0629
        PUSH	IX
        LD	IX,S.WRTVDP
        JR	J0644

J0629:	LD	A,B
        DI
        OUT	(89H),A
        LD	A,C
        OR	80H
        EI
        OUT	(89H),A
        PUSH	HL
        LD	A,B
        LD	B,00H
        LD	HL,RG0SAV
        ADD	HL,BC
        LD	(HL),A
        POP	HL
        RET

;	Subroutine	INITXT
;	Inputs		________________________
;	Outputs		________________________


J063E:	PUSH	IX
        LD	IX,S.INITXT
J0644:	JP	J0295

;	Subroutine	INIT32
;	Inputs		________________________
;	Outputs		________________________


J0647:	PUSH	IX
        LD	IX,S.INIT32
        JR	J0644

;	Subroutine	INIGRP
;	Inputs		________________________
;	Outputs		________________________


J064F:	PUSH	IX
        LD	IX,S.INIGRP
        JR	J0644

;	Subroutine	INIMLT
;	Inputs		________________________
;	Outputs		________________________


J0657:	PUSH	IX
        LD	IX,S.INIMLT
        JR	J0644

;	Subroutine	SETTXT
;	Inputs		________________________
;	Outputs		________________________


J065F:	PUSH	IX
        LD	IX,S.SETTXT
        JR	J0644

;	Subroutine	SETT32
;	Inputs		________________________
;	Outputs		________________________


J0667:	PUSH	IX
        LD	IX,S.SETT32
        JR	J0644

;	Subroutine	SETGRP
;	Inputs		________________________
;	Outputs		________________________


J066F:	PUSH	IX
        LD	IX,S.SETGRP
        JR	J0644

;	Subroutine	SETMLT
;	Inputs		________________________
;	Outputs		________________________


J0677:	PUSH	IX
        LD	IX,S.SETMLT
        JR	J0644

;	Subroutine	CLRSPR
;	Inputs		________________________
;	Outputs		________________________


J067F:	PUSH	IX
        LD	IX,S.CLRSPR
        JR	J0644

;	Subroutine	setup for INIR/OTIR
;	Inputs		HL = vram adres, DE = ram adres, BC = size
;	Outputs		DE = vram adres, HL = ram adres, B = initial repeatcount, A = loopcount

C0687:	EX	DE,HL
        LD	A,C
        OR	A
        LD	A,B
        LD	B,C
        RET	Z
        INC	A
        RET

;	Subroutine	convert base vram adres to screenpage vram adres
;	Inputs		HL = base vram adres, D = active screenpage
;	Outputs		DHL = vram adres for screenpage

C068F:	EX	DE,HL
        LD	A,(SCRMOD)
        SUB	05H	; 5
        JR	C,J06A6
        AND	02H	; 2
        LD	A,H
        JR	NZ,J069D
        RRA
J069D:	RRA
        LD	H,A
        LD	A,00H
        LD	L,A
        ADC	A,A
        ADD	HL,DE
        LD	D,A
        RET

J06A6:	LD	D,00H
        RET

;	Subroutine	CHKNEW (check if new style screenmode)
;	Inputs		________________________
;	Outputs		________________________


C06A9:	PUSH	BC
        LD	B,A
        LD	A,(SCRMOD)
        CP	05H	; 5
        LD	A,B
        POP	BC
        RET

;	Subroutine	NSTWRT
;	Inputs		HL=vram adres (range:0-0FFFFH)
;	Outputs		________________________

C06B3:	PUSH	BC
        PUSH	DE
        PUSH	HL
        LD	A,(ACPAGE)
        AND	A
        LD	D,A
        CALL	NZ,C068F
        LD	A,H
        AND	3FH	; "?"
        OR	40H	; "@"
        JR	J06D3

;	Subroutine	NSETRD
;	Inputs		HL=vram adres (range:0-0FFFFH)
;	Outputs		________________________

C06C5:	PUSH	BC
        PUSH	DE
        PUSH	HL
        LD	A,(ACPAGE)
        AND	A
        LD	D,A
        CALL	NZ,C068F
        LD	A,H
        AND	3FH	; "?"
J06D3:	PUSH	AF
        LD	A,H
        AND	0C0H
        OR	D
        RLCA
        RLCA
        DI
        OUT	(89H),A
        LD	A,8EH
        OUT	(89H),A
        LD	A,L
        OUT	(89H),A
        POP	AF
        EI
        OUT	(89H),A
        POP	HL
        POP	DE
        POP	BC
        RET

;	Subroutine	CALPAT
;	Inputs		________________________
;	Outputs		________________________


J06EC:	LD	L,A
        LD	H,00H
        ADD	HL,HL
        ADD	HL,HL
        ADD	HL,HL
        CALL	C070C
        CP	08H	; 8
        JR	Z,J06FB
        ADD	HL,HL
        ADD	HL,HL
J06FB:	EX	DE,HL
        LD	HL,(PATBAS)
        ADD	HL,DE
        RET

;	Subroutine	CALATR
;	Inputs		________________________
;	Outputs		________________________


J0701:	LD	L,A
        LD	H,00H
        ADD	HL,HL
        ADD	HL,HL
        EX	DE,HL
        LD	HL,(ATRBAS)
        ADD	HL,DE
        RET

;	Subroutine	GSPSIZ
;	Inputs		________________________
;	Outputs		________________________


C070C:	LD	A,(RG1SAV)
        RRCA
        RRCA
        LD	A,08H
        RET	NC
        LD	A,20H
        RET

;	Subroutine	Initialize character pattern table
;	Inputs		________________________
;	Outputs		________________________
;	Remark		Unused code, same code is in subrom
;			code was used in INITXT and INIT32, but these too are moved in subrom

?0717:	CALL	H.INIP
        LD	HL,(CGPBAS)
        CALL	C07F4
        LD	A,(CGPNT+0)
        LD	HL,(CGPNT+1)
        LD	BC,256*8
        PUSH	AF
J072A:	POP	AF
        PUSH	AF
        PUSH	BC
        DI
        CALL	C01F5
        EI
        POP	BC
        OUT	(88H),A
        INC	HL
        DEC	BC
        LD	A,C
        OR	B
        JR	NZ,J072A
        POP	AF
        RET

;	Subroutine	LDIRMV (vram to memory)
;	Inputs		HL = vram adres, DE = ram adres, BC = size
;	Outputs		________________________


C073D:	LD	A,(SCRMOD)
        CP	04H	; 4
        JR	NC,J075A
        LD	A,(MODE)
        AND	08H	; 8
        JR	NZ,J075A
        CALL	C0808
        EX	(SP),HL
        EX	(SP),HL
J0750:	IN	A,(88H)
        LD	(DE),A
        INC	DE
        DEC	BC
        LD	A,C
        OR	B
        JR	NZ,J0750
        RET

J075A:	CALL	C06C5
        CALL	C0687
        LD	C,88H
        CALL	C1425
        EX	DE,HL
        RET

        DEFS	0769H-$,0

;	Subroutine	NWRVRM
;	Inputs		________________________
;	Outputs		________________________


J0769:	PUSH	AF
        CALL	C06B3
        EX	(SP),HL
        EX	(SP),HL
        POP	AF
        OUT	(88H),A
        RET


        DEFS	0777H-$


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________
;	This routine has same entrypoint as MSX1


C0777:	PUSH	IX
        LD	IX,S.CLS
        JP	J0295

;	Subroutine	LDIRVM (memory to vram)
;	Inputs		HL = RAM adres, DE = VRAM adres, BC = size
;	Outputs		________________________


C0780:	EX	DE,HL
        LD	A,(SCRMOD)
        CP	04H	; 4
        JR	NC,J079C
        LD	A,(MODE)
        AND	08H	; 8
        JR	NZ,J079C
        CALL	C07F4
J0792:	LD	A,(DE)
        OUT	(88H),A
        INC	DE
        DEC	BC
        LD	A,C
        OR	B
        JR	NZ,J0792
        RET

J079C:	CALL	C06B3
        CALL	C0687
        LD	C,88H
        CALL	C141C
        EX	DE,HL
        RET

        DEFS	07ABH-$,0

;	Subroutine	copy charpattern to PATWRK
;	Inputs		A = charcode
;	Outputs		________________________


C07AB:	LD	H,00H
        LD	L,A
        ADD	HL,HL
        ADD	HL,HL
        ADD	HL,HL
        EX	DE,HL
        LD	HL,(CGPNT+1)
        ADD	HL,DE
        LD	DE,PATWRK
        LD	B,08H	; 8
J07BB:	PUSH	HL
        PUSH	DE
        PUSH	BC
        LD	A,(CGPNT+0)
        CALL	C01F5
        EI
        POP	BC
        POP	DE
        POP	HL
        LD	(DE),A
        INC	DE
        INC	HL
        DJNZ	J07BB
        RET

;	Subroutine	do cls
;	Inputs		________________________
;	Outputs		________________________


C07CE:	PUSH	IX
        LD	IX,S.CLRTXT
        JP	J0295

;	Subroutine	WRTVRM
;	Inputs		________________________
;	Outputs		________________________


C07D7:	PUSH	AF
        CALL	C07F4
        EX	(SP),HL
        EX	(SP),HL
        POP	AF
        OUT	(88H),A
        RET

;	Subroutine	RDVRM
;	Inputs		________________________
;	Outputs		________________________


C07E1:	CALL	C0808
        EX	(SP),HL
        EX	(SP),HL
        IN	A,(88H)
        RET

;	Subroutine	NRDVRM
;	Inputs		________________________
;	Outputs		________________________


J07E9:	CALL	C06C5
        EX	(SP),HL
        EX	(SP),HL
        IN	A,(88H)
        RET

;	Subroutine	set VDP for VRAM write at screenlocation
;	Inputs		H=x pos, L=y pos
;	Outputs		________________________


C07F1:	CALL	C0B98

;	Subroutine	SETWRT
;	Inputs		HL=vram adres
;	Outputs		________________________


C07F4:	XOR	A
        DI
        OUT	(89H),A
        LD	A,8EH
        OUT	(89H),A
        LD	A,L
        OUT	(89H),A
        LD	A,H
        AND	3FH	; "?"
        OR	40H	; "@"
        OUT	(89H),A
        EI
        RET

;	Subroutine	SETRD
;	Inputs		________________________
;	Outputs		________________________


C0808:	XOR	A
        DI
        OUT	(89H),A
        LD	A,8EH
        OUT	(89H),A
        LD	A,L
        OUT	(89H),A
        LD	A,H
        AND	3FH	; "?"
        OUT	(89H),A
        EI
        RET

;	Subroutine	CHGCLR
;	Inputs		________________________
;	Outputs		________________________


J081A:	LD	A,(SCRMOD)
        DEC	A
        JP	M,J0881
        PUSH	AF
        CALL	C088E
        POP	AF
        RET	NZ
        LD	A,(FORCLR)
        ADD	A,A
        ADD	A,A
        ADD	A,A
        ADD	A,A
        LD	HL,BAKCLR
        OR	(HL)
        LD	HL,(T32COL)
        LD	BC,32

;	Subroutine	BIGFIL
;	Inputs		________________________
;	Outputs		________________________


C0838:	PUSH	AF
        JR	J0841

;	Subroutine	FILVRM
;	Inputs		HL=vram adres,BC=size,A=data
;	Outputs		________________________


J083B:	PUSH	AF
        CALL	C06A9
        JR	C,J0853
J0841:	CALL	C06B3
        LD	A,C
        OR	A
        JR	Z,J0849
        INC	B
J0849:	POP	AF
J084A:	OUT	(88H),A
        DEC	C
        JP	NZ,J084A
        DJNZ	J084A
        RET

J0853:	POP	AF
        PUSH	HL
        PUSH	DE
        LD	E,A
        LD	A,H
        AND	3FH	; "?"
        LD	H,A
        PUSH	HL
        ADD	HL,BC
        DEC	HL
        LD	A,H
        CP	40H	; "@"
        POP	HL
        JR	C,J087A
        PUSH	BC
        XOR	A
        SUB	L
        LD	C,A
        LD	A,40H	; "@"
        SBC	A,H
        LD	B,A
        LD	A,E
        CALL	C0838
        POP	BC
        ADD	HL,BC
        LD	C,L
        LD	A,H
        SUB	40H	; "@"
        LD	B,A
        LD	HL,0
J087A:	LD	A,E
        CALL	C0838
        POP	DE
J087F:	POP	HL
        RET


J0881:	LD	A,(FORCLR)
        ADD	A,A
        ADD	A,A
        ADD	A,A
        ADD	A,A
        LD	HL,BAKCLR
        OR	(HL)
        JR	J0891


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C088E:	LD	A,(BDRCLR)
J0891:	LD	B,A
        LD	C,07H	; 7
        JP	J0612

;	Subroutine	CLS
;	Inputs		________________________
;	Outputs		________________________


J0897:	RET	NZ
        PUSH	HL
        CALL	C0777
        POP	HL
        RET

;	Subroutine	TOTEXT
;	Inputs		________________________
;	Outputs		________________________


C089E:	CALL	C0B4E
        RET	C
        LD	A,(OLDSCR)
        CALL	H.TOTE
        PUSH	IX
        LD	IX,S.CHGMDP
        JP	J0295

;	Subroutine	CHGMOD
;	Inputs		________________________
;	Outputs		________________________


J08B1:	PUSH	IX
        LD	IX,S.CHGMOD
        JP	J0295

;	Subroutine	LPTOUT
;	Inputs		________________________
;	Outputs		________________________


C08BA:	CALL	H.LPTO
        PUSH	AF
J08BE:	CALL	C0564
        JR	C,J08D5
        CALL	C08E1
        JR	Z,J08BE
        POP	AF

;	Subroutine	write byte to printerport
;	Inputs		________________________
;	Outputs		________________________


C08C9:	PUSH	AF
        OUT	(91H),A
        XOR	A
        OUT	(90H),A
        DEC	A
        OUT	(90H),A
        POP	AF
        AND	A
        RET

J08D5:	XOR	A
        LD	(LPTPOS),A
        LD	A,0DH	; 13
        CALL	C08C9
        POP	AF
        SCF
        RET

;	Subroutine	LPTSTT
;	Inputs		________________________
;	Outputs		________________________


C08E1:	CALL	H.LPTS
        IN	A,(90H)
        RRCA
        RRCA
        CCF
        SBC	A,A
        RET

;	Subroutine	POSIT
;	Inputs		________________________
;	Outputs		________________________


C08EB:	LD	A,1BH
        RST	18H
        LD	A,59H	; "Y"
        RST	18H
        LD	A,L
        ADD	A,1FH
        RST	18H
        LD	A,H
        ADD	A,1FH
        RST	18H
        RET

;	Subroutine	CNVCHR
;	Inputs		________________________
;	Outputs		________________________


C08FA:	PUSH	HL
        PUSH	AF
        LD	HL,GRPHED
        XOR	A
        CP	(HL)
        LD	(HL),A
        JR	Z,J0911
        POP	AF
        SUB	40H	; "@"
        CP	20H	; " "
        JR	C,J090F
        ADD	A,40H	; "@"
J090D:	CP	A
        SCF
J090F:	POP	HL
        RET

J0911:	POP	AF
        CP	01H	; 1
        JR	NZ,J090D
        LD	(HL),A
        POP	HL
        RET

;	Subroutine	CHPUT
;	Inputs		________________________
;	Outputs		________________________


J0919:	PUSH	HL
        PUSH	DE
        PUSH	BC
        PUSH	AF
        CALL	H.CHPU
        CALL	C0B4E
        JR	NC,J0937
        CALL	C0A8B
        POP	AF
        PUSH	AF
        CALL	C093C
        CALL	C0A3E
        LD	A,(CSRX)
        DEC	A
        LD	(TTYPOS),A
J0937:	POP	AF
J0938:	POP	BC
        POP	DE
        POP	HL
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C093C:	CALL	C08FA
        RET	NC
        LD	C,A
        JR	NZ,J0950
        LD	HL,ESCCNT
        LD	A,(HL)
        AND	A
        JP	NZ,J09EC
        LD	A,C
        CP	20H	; " "
        JR	C,J0971
J0950:	LD	HL,(CSRY)
        CP	7FH
        JP	Z,J0AFA
        CALL	C0B8D
        CALL	C0AA1
        RET	NZ
        XOR	A
        CALL	C0BDB
        LD	H,01H

;	Subroutine	screencode LF
;	Inputs		________________________
;	Outputs		________________________


C0965:	CALL	C0ABE
        RET	NZ
        CALL	C0AC6
        LD	L,01H	; 1
        JP	C0AE5

J0971:	LD	HL,I098C-2
        LD	C,0CH	; 12


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C0976:	INC	HL
        INC	HL
        AND	A
        DEC	C
        RET	M
        CP	(HL)
        INC	HL
        JR	NZ,C0976
        LD	C,(HL)
        INC	HL
        LD	B,(HL)
        LD	HL,(CSRY)
        CALL	C098A
        XOR	A
        RET

;	Subroutine	call subroutine
;	Inputs		BC = adres of subroutine
;	Outputs		________________________


C098A:	PUSH	BC
        RET

I098C:	DEFB	7
        DEFW	C1113			; BEEP
        DEFB	8
        DEFW	C0AA9			; BS, cursor left with warp
        DEFB	9
        DEFW	C0ACE
        DEFB	10
        DEFW	C0965
        DEFB	11
        DEFW	C0ADC
        DEFB	12
        DEFW	C07CE			; do CLS
        DEFB	13
        DEFW	C0ADE
        DEFB	27
        DEFW	C09E6
        DEFB	28
        DEFW	C0AB8			; RIGHT, cursor right with linewarp
        DEFB	29
        DEFW	C0AA9			; LEFT, cursor left with linewarp
        DEFB	30
        DEFW	C0AB4			; UP, cursor up if possible
        DEFB	31
        DEFW	C0ABE			; DOWN, cursor down if possible

; ESC table

I09B0:	DEFB	"j"
        DEFW	C07CE			; ESC j, clear screen
        DEFB	"E"
        DEFW	C07CE			; ESC E, clear screen
        DEFB	"K"
        DEFW	C0B05			; ESC K, clear to end of line
        DEFB	"J"
        DEFW	C0B19			; ESC J, clear to end of screen
        DEFB	"l"
        DEFW	C0B03			; ESC l, clear line
        DEFB	"L"
        DEFW	C0AEE			; ESC L, insert line
        DEFB	"M"
        DEFW	C0AE2			; ESC M, delete line
        DEFB	"Y"
        DEFW	C09E3			; ESC Y, locate cursor
        DEFB	"A"
        DEFW	C0AB4			; ESC A, cursor up if possible
        DEFB	"B"
        DEFW	C0ABE			; ESC B, cursor down if possible
        DEFB	"C"
        DEFW	C0AA1			; ESC C, cursor right if possible
        DEFB	"D"
        DEFW	C0AB2			; ESC D, cursor left if possible
        DEFB	"H"
        DEFW	C0ADC			; ESC H, cursor home
        DEFB	"x"
        DEFW	C09DD			; ESC x
        DEFB	"y"
        DEFW	C09E0			; ESC y

; ESC x

C09DD:	LD	A,1
        DEFB	001H			; LD BC,xxxx (skip next instruction)

; ESC y

C09E0:	LD	A,2
        DEFB	001H			; LD BC,xxxx (skip next instruction)

; ESC Y

C09E3:	LD	A,4
        DEFB	001H			; LD BC,xxxx (skip next instruction)

; ESC handler

C09E6:	LD	A,0FFH
        LD	(ESCCNT),A		; mark start of ESC sequence
        RET

J09EC:	JP	P,J09FA
        LD	(HL),00H
        LD	A,C
        LD	HL,I09B0-2
        LD	C,0FH	; 15
        JP	C0976

J09FA:	DEC	A
        JR	Z,J0A1B
        DEC	A
        JR	Z,J0A25
        DEC	A
        LD	(HL),A
        LD	A,(LINLEN)
        LD	DE,CSRX
        JR	Z,J0A10
        LD	(HL),03H	; 3
        CALL	C0BE2
        DEC	DE
J0A10:	LD	B,A
        LD	A,C
        SUB	20H	; " "
        CP	B
        INC	A
        LD	(DE),A
        RET	C
        LD	A,B
        LD	(DE),A
        RET

; ESC x handler

J0A1B:	LD	(HL),A
        LD	A,C
        SUB	34H	; "4"
        JR	Z,J0A2C
        DEC	A
        JR	Z,J0A33
        RET

; ESC y handler

J0A25:	LD	(HL),A
        LD	A,C
        SUB	34H	; "4"
        JR	NZ,J0A30
        INC	A
J0A2C:	LD	(CSTYLE),A
        RET

J0A30:	DEC	A
        RET	NZ
        INC	A
J0A33:	LD	(CSRSW),A
        RET

;	Subroutine	display cursor when disabled
;	Inputs		________________________
;	Outputs		________________________


C0A37:	LD	A,(CSRSW)
        AND	A
        RET	NZ
        JR	J0A43

;	Subroutine	display cursor when enabled
;	Inputs		________________________
;	Outputs		________________________


C0A3E:	LD	A,(CSRSW)
        AND	A
        RET	Z
J0A43:	CALL	H.DSPC
        CALL	C0B4E
        RET	NC
        LD	HL,(CSRY)
        PUSH	HL
        CALL	C0B83
        LD	(CURSAV),A
        LD	L,A
        LD	H,00H
        ADD	HL,HL
        ADD	HL,HL
        ADD	HL,HL
        EX	DE,HL
        LD	HL,(CGPBAS)
        PUSH	HL
        ADD	HL,DE
        CALL	C0B54
        LD	HL,LINWRK+7
        LD	B,08H	; 8
        LD	A,(CSTYLE)
        AND	A
        JR	Z,J0A70
        LD	B,03H	; 3
J0A70:	LD	A,(HL)
        CPL
        LD	(HL),A
        DEC	HL
        DJNZ	J0A70
        POP	HL
        LD	BC,07F8H
        ADD	HL,BC
        CALL	C0B6B
        POP	HL
        LD	C,0FFH
        JP	C0B8D

;	Subroutine	remove cursor when disabled
;	Inputs		________________________
;	Outputs		________________________


C0A84:	LD	A,(CSRSW)
        AND	A
        RET	NZ
        JR	J0A90

;	Subroutine	remove cursor when enabled
;	Inputs		________________________
;	Outputs		________________________


C0A8B:	LD	A,(CSRSW)
        AND	A
        RET	Z
J0A90:	CALL	H.ERAC
        CALL	C0B4E
        RET	NC
        LD	HL,(CSRY)
        LD	A,(CURSAV)
        LD	C,A
        JP	C0B8D

;	Subroutine	move cursor right if possible
;	Inputs		________________________
;	Outputs		________________________


C0AA1:	LD	A,(LINLEN)
        CP	H
        RET	Z
        INC	H
        JR	C0AC6

;	Subroutine	move cursor left with linewarp
;	Inputs		________________________
;	Outputs		________________________


C0AA9:	CALL	C0AB2
        RET	NZ
        LD	A,(LINLEN)
        LD	H,A
        DEFB	011H			; LD DE,xxxx (skip next 2 instructions), so cursor line up

;	Subroutine	move cursor left if possible
;	Inputs		________________________
;	Outputs		________________________


C0AB2:	DEC	H
        DEFB	03EH			; LD A,xx (skip next instruction)

;	Subroutine	move cursor up if possible
;	Inputs		________________________
;	Outputs		________________________


C0AB4:	DEC	L
        RET	Z			; already at top of screen, quit
        JR	C0AC6			; update cursorpos

;	Subroutine	move cursor right with linewarp
;	Inputs		________________________
;	Outputs		________________________


C0AB8:	CALL	C0AA1			; cursor right if possible
        RET	NZ			; was possible, quit
        LD	H,1			; start of line and linedown

;	Subroutine	cursor down if possible
;	Inputs
;	Outputs		________________________


C0ABE:	CALL	C0BE2
        CP	L
        RET	Z
        JR	C,J0ACA
        INC	L


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C0AC6:	LD	(CSRY),HL
        RET

J0ACA:	DEC	L
        XOR	A
        JR	C0AC6

C0ACE:	LD	A,20H	; " "
        CALL	C093C
        LD	A,(CSRX)
        DEC	A
        AND	07H	; 7
        JR	NZ,C0ACE
        RET

C0ADC:	LD	L,01H	; 1

;	Subroutine	screencode CR
;	Inputs		________________________
;	Outputs		________________________


C0ADE:	LD	H,01H	; 1
        JR	C0AC6

C0AE2:	CALL	C0ADE

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C0AE5:	PUSH	IX
        LD	IX,S.DELLNO
        JP	J0295

C0AEE:	CALL	C0ADE

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C0AF1:	PUSH	IX
        LD	IX,S.INSLNO
        JP	J0295

J0AFA:	CALL	C0AA9
        RET	Z
        LD	C,20H	; " "
        JP	C0B8D

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C0B03:	LD	H,01H	; 1

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C0B05:	CALL	C0BD9
        PUSH	HL
        CALL	C07F1
        POP	HL
J0B0D:	LD	A,20H	; " "
        OUT	(88H),A
        INC	H
        LD	A,(LINLEN)
        CP	H
        JR	NC,J0B0D
        RET

C0B19:	PUSH	HL
        CALL	C0B05
        POP	HL
        CALL	C0BE2
        CP	L
        RET	C
        RET	Z
        LD	H,01H	; 1
        INC	L
        JR	C0B19

;	Subroutine	ERAFNK
;	Inputs		________________________
;	Outputs		________________________


J0B29:	CALL	H.ERAF
        XOR	A
        CALL	C0B4B
        RET	NC
        PUSH	HL
        LD	HL,(CRTCNT)
        CALL	C0B03
        POP	HL
        RET

;	Subroutine	FNKSB
;	Inputs		________________________
;	Outputs		________________________


J0B3A:	LD	A,(CNSDFG)
        AND	A
        RET	Z

;	Subroutine	DSPFNK
;	Inputs		________________________
;	Outputs		________________________


C0B3F:	CALL	H.DSPF
        PUSH	IX
        LD	IX,S.DSPFNK
        JP	J0295

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C0B4B:	LD	(CNSDFG),A

;	Subroutine	check if in textmode
;	Inputs		________________________
;	Outputs		Cx set if in textmode

C0B4E:	LD	A,(SCRMOD)
        CP	02H	; 2
        RET

;	Subroutine	copy 8 bytes of vram to LINWRK
;	Inputs		HL=vram adres
;	Outputs		________________________


C0B54:	PUSH	HL
        LD	C,08H	; 8
        JR	J0B61

;	Subroutine	copy screenline to LINWRK
;	Inputs		L=linenumber
;	Outputs		________________________
;	Remark		Unused code, code is in subrom
;			This code will not even work if screen is in 80 kol mode because LINWRK is only 40 bytes!

?0B59:	PUSH	HL
        CALL	C0B96
        LD	A,(LINLEN)
        LD	C,A
J0B61:	LD	B,00H
        LD	DE,LINWRK
        CALL	C073D
        POP	HL
        RET

;	Subroutine	copy 8 bytes from LINWRK to vram
;	Inputs		HL=vram adres
;	Outputs		________________________


C0B6B:	PUSH	HL
        LD	C,08H	; 8
        JR	J0B78

;	Subroutine	copy LINWRK to screenline
;	Inputs		L=linenumber
;	Outputs		________________________
;	Remark		Unused code, code is in subrom
;			This code will not even work if screen is in 80 kol mode because LINWRK is only 40 bytes!

?0B70:	PUSH	HL
        CALL	C0B96
        LD	A,(LINLEN)
        LD	C,A
J0B78:	LD	B,00H
        EX	DE,HL
        LD	HL,LINWRK
        CALL	C0780
        POP	HL
        RET

;	Subroutine	read char from screenlocation
;	Inputs		H=x pos, L=y pos
;	Outputs		A=char, C=char

C0B83:	PUSH	HL
        CALL	C0B98
        CALL	C07E1
        LD	C,A
        POP	HL
        RET

;	Subroutine	write char at screenlocation
;	Inputs		H=x pos, L=y pos, C=char
;	Outputs		________________________


C0B8D:	PUSH	HL
        CALL	C07F1
        LD	A,C
        OUT	(88H),A
        POP	HL
        RET

;	Subroutine	convert linenumber to VRAM adres
;	Inputs		L=y pos
;	Outputs		HL=vram adres in nametable

C0B96:	LD	H,01H	; 1

;	Subroutine	convert screenlocation to VRAM adres
;	Inputs		H=x pos, L=y pos
;	Outputs		HL=vram adres in nametable

C0B98:	PUSH	BC
        DEC	H
        DEC	L
        LD	E,H
        LD	H,00H
        LD	D,H
        ADD	HL,HL
        ADD	HL,HL
        ADD	HL,HL
        LD	C,L
        LD	B,H
        ADD	HL,HL
        ADD	HL,HL
        LD	A,(SCRMOD)
        AND	A
        LD	A,(LINLEN)
        JR	Z,J0BB3
        SBC	A,22H	; """
        JR	J0BC0

J0BB3:	CP	29H	; ")"
        JR	C,J0BBD
        ADD	HL,BC
        ADD	HL,HL
        SBC	A,52H	; "R"
        JR	J0BC0

J0BBD:	ADD	HL,BC
        SBC	A,2AH	; "*"
J0BC0:	ADD	HL,DE
        CPL
        AND	A
        RRA
        LD	E,A
        ADD	HL,DE
        EX	DE,HL
        LD	HL,(NAMBAS)
        ADD	HL,DE
        POP	BC
        RET

;	Subroutine	get LINTTB entry
;	Inputs		L = linenumber
;	Outputs		DE = LINTTB entry for line, A = , Zx when the one physiscal line is the succession of the line above

C0BCD:	PUSH	HL
        LD	DE,LINTTB-1
        LD	H,00H
        ADD	HL,DE
        LD	A,(HL)
        EX	DE,HL
        POP	HL
        AND	A
        RET

;	Subroutine	unmark line as succession
;	Inputs		L = linenumber
;	Outputs		________________________


C0BD9:	DEFB	03EH			; LD A,xx (skip next instruction)
C0BDA:	XOR	A

;	Subroutine	__________________________
;	Inputs		________________________
;	Outputs		________________________

C0BDB:	PUSH	AF
        CALL	C0BCD
        POP	AF
        LD	(DE),A
        RET

;	Subroutine	get max linenumber
;	Inputs		________________________
;	Outputs		________________________


C0BE2:	LD	A,(CNSDFG)
        PUSH	HL
        LD	HL,CRTCNT
        ADD	A,(HL)
        POP	HL
        RET

;	Subroutine	SNSMAT
;	Inputs		________________________
;	Outputs		________________________


J0BEC:	LD	C,A
        DI
        IN	A,(0AAH)
        AND	0F0H
        ADD	A,C
        OUT	(0AAH),A
        EI
        IN	A,(0A9H)
        RET

;	Subroutine	ISFLIO
;	Inputs		________________________
;	Outputs		________________________


C0BF9:	CALL	H.ISFL
        PUSH	HL
        LD	HL,(PTRFIL)
        LD	A,L
        OR	H
        POP	HL
        RET

;	Subroutine	DCOMPR
;	Inputs		________________________
;	Outputs		________________________


J0C04:	LD	A,H
        SUB	D
        RET	NZ
        LD	A,L
        SUB	E
        RET

;	Subroutine	GETVCP
;	Inputs		________________________
;	Outputs		________________________


C0C0A:	LD	L,02H	; 2
        JR	C0C11

;	Subroutine	GETVC2
;	Inputs		________________________
;	Outputs		________________________


J0C0E:	LD	A,(VOICEN)


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C0C11:	PUSH	DE
        LD	DE,VCBA
        LD	H,00H
        ADD	HL,DE
        OR	A
        JR	Z,J0C22
        LD	DE,0025H
J0C1E:	ADD	HL,DE
        DEC	A
        JR	NZ,J0C1E
J0C22:	POP	DE
        RET

; The following NOP really puzzels me
; It has nothing to do with compatibility to MSX1 or so
; Maybe is just leftover space and 0C25 was pushed against 0C3C
        DEFS	0C25H-$,0

J0C25:	LD	A,(NEWKEY+6)
        RRCA
        JR	C,J0C34
        XOR	A
        LD	(CHRCNT),A
        INC	A
        SET	0,(HL)
        JR	J0C39

J0C34:	LD	A,0FFH
J0C36:	LD	(KANAST),A

J0C39:	JP	J0F29

;	Subroutine	KEYINT
;	Inputs		________________________
;	Outputs		________________________
; 	Remark		Entry at the same adres as MSX1

J0C3C:	PUSH	HL
        PUSH	DE
        PUSH	BC
        PUSH	AF
        EXX
        EX	AF,AF'
        PUSH	HL
        PUSH	DE
        PUSH	BC
        PUSH	AF
        PUSH	IY
        PUSH	IX
        CALL	H.KEYI
        CALL	C1479
        JP	P,J0D02
        CALL	H.TIMI
        EI
        LD	(STATFL),A
        AND	20H	; " "
        LD	HL,TRPTBL+11*3
        CALL	NZ,C0EF1
        LD	HL,(INTCNT)
        DEC	HL
        LD	A,H
        OR	L
        JR	NZ,J0C73
        LD	HL,TRPTBL+17*3
        CALL	C0EF1
        LD	HL,(INTVAL)
J0C73:	LD	(INTCNT),HL
        LD	HL,(JIFFY)
        INC	HL
        LD	(JIFFY),HL
        LD	A,(MUSICF)
        LD	C,A
        XOR	A
J0C82:	RR	C
        PUSH	AF
        PUSH	BC
        CALL	C,C1131
        POP	BC
        POP	AF
        INC	A
        CP	03H	; 3
        JR	C,J0C82
        LD	HL,SCNCNT
        DEC	(HL)
        JR	NZ,J0D02

        IF INTHZ EQ 60
        LD	(HL),2			; scanfrequency 2*1000/60 = 33 ms
        ELSE
        LD	(HL),1			; scanfrequency 1*1000/50 = 20 ms
        ENDIF

        XOR	A
        CALL	C1202
        AND	30H	; "0"
        PUSH	AF
        LD	A,01H	; 1
        CALL	C1202
        AND	30H	; "0"
        RLCA
        RLCA
        POP	BC
        OR	B
        PUSH	AF
        CALL	C121C
        AND	01H	; 1
        POP	BC
        OR	B
        LD	C,A
        LD	HL,TRGFLG
        XOR	(HL)
        AND	(HL)
        LD	(HL),C
        LD	C,A
        RRCA
        LD	HL,TRPTBL+12*3
        CALL	C,C0EF1
        RL	C
        LD	HL,TRPTBL+16*3
        CALL	C,C0EF1
        RL	C
        LD	HL,TRPTBL+14*3
        CALL	C,C0EF1
        RL	C
        LD	HL,TRPTBL+15*3
        CALL	C,C0EF1
        RL	C
        LD	HL,TRPTBL+13*3
        CALL	C,C0EF1
        XOR	A
        LD	(CLIKFL),A
        CALL	C0D12
        JR	NZ,J0D02
        LD	HL,REPCNT
        DEC	(HL)
        JR	NZ,J0D02
        LD	(HL),1
        LD	HL,OLDKEY+0
        LD	DE,OLDKEY+1
        LD	BC,11-1
        LD	(HL),0FFH
        LDIR
        CALL	C0D4E
J0D02:	POP	IX
        POP	IY
        POP	AF
        POP	BC
        POP	DE
        POP	HL
        EX	AF,AF'
        EXX
        POP	AF
        POP	BC
        POP	DE
        POP	HL
        EI
        RET

;	Subroutine	scan keyboard and translate to keycodes
;	Inputs		________________________
;	Outputs		Zx set if keyboardbuffer is empty
;	Remark		Entry at the same adres as MSX1

C0D12:	IN	A,(0AAH)
        AND	0F0H
        LD	C,A
        LD	B,0BH	; 11
        LD	HL,NEWKEY
J0D1C:	LD	A,C
        OUT	(0AAH),A
        IN	A,(0A9H)
        LD	(HL),A
        INC	C
        INC	HL
        DJNZ	J0D1C
        LD	A,(ENSTOP)
        AND	A
        JR	Z,J0D3A
        LD	A,(NEWKEY+6)
        CP	0E8H
        JR	NZ,J0D3A
        LD	IX,M409B
        JP	C02BF

J0D3A:	LD	DE,NEWKEY
        LD	B,0BH	; 11
J0D3F:	DEC	DE
        DEC	HL
        LD	A,(DE)
        CP	(HL)
        JR	NZ,J0D49
        DJNZ	J0D3F
        JR	C0D4E

J0D49:
        IF INTHZ EQ 60
        LD	A,14H			; autorepeat at 20 keyboardscans (= 666 ms)
        ELSE
        LD	A,20H			; autorepeat at 32 keyboardscans (= 640 ms)
        ENDIF

        LD	(REPCNT),A

;	Subroutine	scan for transitions and translate to keycodes
;	Inputs		________________________
;	Outputs		________________________


C0D4E:	LD	B,0BH	; 11
        LD	HL,OLDKEY
        LD	DE,NEWKEY
J0D56:	LD	A,(DE)
        LD	C,A
        XOR	(HL)
        AND	(HL)
        LD	(HL),C
        CALL	NZ,C0D89
        INC	DE
        INC	HL
        DJNZ	J0D56

;	Subroutine	check if keyboardbuffer is empty
;	Inputs		________________________
;	Outputs		________________________


C0D62:	LD	HL,(GETPNT)
        LD	A,(PUTPNT)
        SUB	L
        RET

;	Subroutine	CHSNS
;	Inputs		________________________
;	Outputs		________________________
;	Remark		Entry at the same adres as MSX1

C0D6A:	EI
        PUSH	HL
        PUSH	DE
        PUSH	BC
        CALL	C0B4E
        JR	NC,J0D82
        LD	A,(FNKSWI)
        LD	HL,NEWKEY+6
        XOR	(HL)
        LD	HL,CNSDFG
        AND	(HL)
        RRCA
        CALL	C,C0B3F
J0D82:	CALL	C0D62
        POP	BC
        POP	DE
        POP	HL
        RET

;	Subroutine	convert to scancode and handle
;	Inputs		________________________
;	Outputs		________________________
;	Remark		Entry at the same adres as MSX1

C0D89:	PUSH	HL
        PUSH	DE
        PUSH	BC
        PUSH	AF
        LD	A,0BH	; 11
        SUB	B
        ADD	A,A
        ADD	A,A
        ADD	A,A
        LD	C,A
        LD	B,08H	; 8
        POP	AF
J0D97:	RRA
        PUSH	BC
        PUSH	AF
        CALL	C,K.HAND
        POP	AF
        POP	BC
        INC	C
        DJNZ	J0D97
        JP	J0938


        IF	KEYTYP EQ 0
        INCLUDE	KEYJAP
        ENDIF


        IF	KEYTYP EQ 1
        INCLUDE KEYINT
        ENDIF

        IF	KEYTYP EQ 2
        INCLUDE KEYFR
        ENDIF

        IF	KEYTYP EQ 3
        INCLUDE KEYUK
        ENDIF

        IF	KEYTYP EQ 4
        INCLUDE KEYGER
        ENDIF

        IF	KEYTYP EQ 5
        INCLUDE KEYRUS
        ENDIF

        IF	KEYTYP EQ 6
        INCLUDE KEYSPA
        ENDIF

        ORG	010C2H

;	Subroutine	increase keyboardbuffer pointer
;	Inputs		________________________
;	Outputs		________________________


C10C2:	INC	HL
        LD	A,L
        CP	18H
        RET	NZ
        LD	HL,KEYBUF
        RET

;	Subroutine	CHGET
;	Inputs		________________________
;	Outputs		________________________

C10CB:	PUSH	HL
        PUSH	DE
        PUSH	BC
        CALL	H.CHGE
        CALL	C0D6A
        JR	NZ,J10E1
        CALL	C0A37
J10D9:	CALL	C0D6A
        JR	Z,J10D9
        CALL	C0A84
J10E1:	LD	HL,INTFLG
        LD	A,(HL)
        CP	04H	; 4
        JR	NZ,J10EB
        LD	(HL),00H
J10EB:	LD	HL,(GETPNT)
        LD	C,(HL)
        CALL	C10C2
        LD	(GETPNT),HL
        LD	A,C
        JP	J0938

;	Subroutine	CKCNTC
;	Inputs		________________________
;	Outputs		________________________


J10F9:	PUSH	HL
        LD	HL,0
        CALL	C04F0
        POP	HL
        RET

;	Subroutine	WRTPSG
;	Inputs		________________________
;	Outputs		________________________


C1102:	DI
        OUT	(0A0H),A
        PUSH	AF
        LD	A,E
        EI
        OUT	(0A1H),A
        POP	AF
        RET


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C110C:	LD	A,0EH	; 14

;	Subroutine	RDPSG
;	Inputs		________________________
;	Outputs		________________________


C110E:	OUT	(0A0H),A
        IN	A,(0A2H)
        RET

;	Subroutine	BEEP
;	Inputs		________________________
;	Outputs		________________________


C1113:	PUSH	IX
        LD	IX,S.BEEP
        JP	J0295

J111C:	PUSH	AF
        LD	A,0FH	; 15
        OUT	(0A0H),A
        IN	A,(0A2H)
        AND	7FH
        LD	B,A
        POP	AF
        OR	A
        LD	A,80H
        JR	Z,J112D
        XOR	A
J112D:	OR	B
        OUT	(0A1H),A
        RET


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C1131:	LD	B,A
        CALL	C0C0A
        DEC	HL
        LD	D,(HL)
        DEC	HL
        LD	E,(HL)
        DEC	DE
        LD	(HL),E
        INC	HL
        LD	(HL),D
        LD	A,D
        OR	E
        RET	NZ
        LD	A,B
        LD	(QUEUEN),A
        CALL	C11D8
        CP	0FFH
        JR	Z,J11A6
        LD	D,A
        AND	0E0H
        RLCA
        RLCA
        RLCA
        LD	C,A
        LD	A,D
        AND	1FH
        LD	(HL),A
        CALL	C11D8
        DEC	HL
        LD	(HL),A
        INC	C
J115C:	DEC	C
        RET	Z
        CALL	C11D8
        LD	D,A
        AND	0C0H
        JR	NZ,J1177
        CALL	C11D8
        LD	E,A
        LD	A,B
        RLCA
        CALL	C1102
        INC	A
        LD	E,D
        CALL	C1102
        DEC	C
        JR	J115C

J1177:	LD	H,A
        AND	80H
        JR	Z,J118B
        LD	E,D
        LD	A,B
        ADD	A,08H	; 8
        CALL	C1102
        LD	A,E
        AND	10H	; 16
        LD	A,0DH	; 13
        CALL	NZ,C1102
J118B:	LD	A,H
        AND	40H	; "@"
        JR	Z,J115C
        CALL	C11D8
        LD	D,A
        CALL	C11D8
        LD	E,A
        LD	A,0BH	; 11
        CALL	C1102
        INC	A
        LD	E,D
        CALL	C1102
        DEC	C
        DEC	C
        JR	J115C

J11A6:	LD	A,B
        ADD	A,08H	; 8
        LD	E,00H
        CALL	C1102
        INC	B
        LD	HL,MUSICF
        XOR	A
        SCF
J11B4:	RLA
        DJNZ	J11B4
        AND	(HL)
        XOR	(HL)
        LD	(HL),A

;	Subroutine	STRTMS
;	Inputs		________________________
;	Outputs		________________________


J11BA:	LD	A,(MUSICF)
        OR	A
        RET	NZ
        LD	HL,PLYCNT
        LD	A,(HL)
        OR	A
        RET	Z
        DEC	(HL)
        LD	HL,1
        LD	(VCBA),HL
        LD	(VCBB),HL
        LD	(VCBC),HL
        LD	A,07H	; 7
        LD	(MUSICF),A
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C11D8:	LD	A,(QUEUEN)
        PUSH	HL
        PUSH	DE
        PUSH	BC
        CALL	C14AB
        JP	J0938

;	Subroutine	GTSTCK
;	Inputs		________________________
;	Outputs		________________________


J11E4:	DEC	A
        JP	M,J11F6
        CALL	C1202
        LD	HL,I1229
J11EE:	AND	0FH	; 15
        LD	E,A
        LD	D,00H
        ADD	HL,DE
        LD	A,(HL)
        RET

J11F6:	CALL	C121C
        RRCA
        RRCA
        RRCA
        RRCA
        LD	HL,I1239
        JR	J11EE


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C1202:	LD	B,A
        LD	A,0FH	; 15
        DI
        CALL	C110E
        DJNZ	J1211
        AND	0DFH
        OR	4CH	; "L"
        JR	J1215

J1211:	AND	0AFH
        OR	03H	; 3
J1215:	OUT	(0A1H),A
        CALL	C110C
        EI
        RET

;	Subroutine	read keyboard row 8
;	Inputs		________________________
;	Outputs		________________________


C121C:	DI
        IN	A,(0AAH)
        AND	0F0H
        ADD	A,08H	; 8
        OUT	(0AAH),A
        IN	A,(0A9H)
        EI
        RET

I1229:	DEFB	000H,005H,001H,000H
        DEFB	003H,004H,002H,003H
        DEFB	007H,006H,008H,007H
        DEFB	000H,005H,001H,000H

I1239:	DEFB	000H,003H,005H,004H
        DEFB	001H,002H,000H,003H
        DEFB	007H,000H,006H,005H
        DEFB	008H,001H,007H,000H

;	Subroutine	GTTRIG
;	Inputs		________________________
;	Outputs		________________________


J1249:	DEC	A
        JP	M,J1262
        PUSH	AF
        AND	01H	; 1
        CALL	C1202
        POP	BC
        DEC	B
        DEC	B
        LD	B,10H	; 16
        JP	M,J125D
        LD	B,20H	; " "
J125D:	AND	B
J125E:	SUB	01H	; 1
        SBC	A,A
        RET

J1262:	CALL	C121C
        AND	01H	; 1
        JR	J125E

;	Subroutine	GTPDL
;	Inputs		________________________
;	Outputs		________________________


J1269:	INC	A
        AND	A
        RRA
        PUSH	AF
        LD	B,A
        XOR	A
        SCF
J1270:	RLA
        DJNZ	J1270
        LD	B,A
        POP	AF
        LD	C,10H	; 16
        LD	DE,03AFH
        JR	NC,J1281
        LD	C,20H	; " "
        LD	DE,4C9FH
J1281:	LD	A,0FH	; 15
        DI
        CALL	C110E
        AND	E
        OR	D
        OR	C
        OUT	(0A1H),A
        XOR	C
        OUT	(0A1H),A
        LD	A,0EH	; 14
        OUT	(0A0H),A
        LD	C,00H
J1295:	IN	A,(0A2H)
        AND	B
        JR	Z,J129F
        INC	C
        JP	NZ,J1295
        DEC	C
J129F:	EI
        LD	A,C
        RET


;	Subroutine	GTPAD
;	Inputs		________________________
;	Outputs		________________________


J12A2:	CP	08H	; 8
        JR	C,J12AF
        PUSH	IX
        LD	IX,S.NEWPAD
        JP	J0295

J12AF:	CP	04H	; 4
        LD	DE,0CECH

;	Subroutine	__________________________
;	Inputs		________________________
;	Outputs		________________________

        JR	C,J12BB
        LD	DE,03D3H
        SUB	04H	; 4
J12BB:	DEC	A
        JP	M,J12C8
        DEC	A
        LD	A,(PADX)
        RET	M
        LD	A,(PADY)
        RET	Z
J12C8:	PUSH	AF
        EX	DE,HL
        LD	(FILNAM+0),HL
        SBC	A,A
        CPL
        AND	40H	; "@"
        LD	C,A
        LD	A,0FH	; 15
        DI
        CALL	C110E
        AND	0BFH
        OR	C
        OUT	(0A1H),A
        POP	AF
        JP	M,J12EB
        CALL	C110C
        EI
        AND	08H	; 8
        SUB	01H	; 1
        SBC	A,A
        RET

J12EB:	LD	C,00H
        CALL	C1335
        CALL	C1335
        JR	C,J131D
        CALL	C1323
        JR	C,J131D
        PUSH	DE
        CALL	C1323
        POP	BC
        JR	C,J131D
        LD	A,B
        SUB	D
        JR	NC,J1307
        CPL
        INC	A
J1307:	CP	05H	; 5
        JR	NC,J12EB
        LD	A,C
        SUB	E
        JR	NC,J1311
        CPL
        INC	A
J1311:	CP	05H	; 5
        JR	NC,J12EB
        LD	A,D
        LD	(PADX),A
        LD	A,E
        LD	(PADY),A
J131D:	EI
        LD	A,H
        SUB	01H	; 1
        SBC	A,A
        RET


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C1323:	LD	C,0AH	; 10
        CALL	C1335
        RET	C
        LD	D,L
        PUSH	DE
        LD	C,00H
        CALL	C1335
        POP	DE
        LD	E,L
        XOR	A
        LD	H,A
        RET


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C1335:	CALL	C135E
        LD	B,08H	; 8
        LD	D,C
J133B:	RES	0,D
        RES	2,D
        CALL	C1370
        CALL	C110C
        LD	H,A
        RRA
        RRA
        RRA
        RL	L
        SET	0,D
        SET	2,D
        CALL	C1370
        DJNZ	J133B
        SET	4,D
        SET	5,D
        CALL	C1370
        LD	A,H
        RRA
        RET


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C135E:	LD	A,35H	; "5"
        OR	C
        LD	D,A
        CALL	C1370
J1365:	CALL	C110C
        AND	02H	; 2
        JR	Z,J1365
        RES	4,D
        RES	5,D


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C1370:	PUSH	HL
        PUSH	DE
        LD	HL,(FILNAM+0)
        LD	A,L
        CPL
        AND	D
        LD	D,A
        LD	A,0FH	; 15
        OUT	(0A0H),A
        IN	A,(0A2H)
        AND	L
        OR	D
        OR	H
        OUT	(0A1H),A
        POP	DE
        POP	HL
        RET

;	Subroutine	STMOTR
;	Inputs		________________________
;	Outputs		________________________

J1387:	AND	A
        JP	M,J1395
J138B:	JR	NZ,J1390
        LD	A,09H	; 9
        DEFB	0C2H
J1390:	LD	A,08H
        OUT	(0ABH),A
        RET
J1395:	IN	A,(0AAH)
        AND	10H	; 16
        JR	J138B

;	Subroutine	NMI (NonMaskable Interrupt routine Z80)
;	Inputs		________________________
;	Outputs		________________________


J139B:	CALL	H.NMI
        RETN

;	Subroutine	INIFNK
;	Inputs		________________________
;	Outputs		________________________


J13A0:	LD	HL,FNKSTR
        LD	B,9FH
        PUSH	HL
        XOR	A
J13A7:	LD	(HL),A
        INC	HL
        DJNZ	J13A7
        POP	HL
        LD	B,0AH	; 10
        LD	DE,I13C3
J13B1:	LD	C,10H	; 16
J13B3:	LD	A,(DE)
        INC	DE
        LD	(HL),A
        INC	HL
        DEC	C
        OR	A
        JR	NZ,J13B3
        PUSH	BC
        LD	B,00H
        ADD	HL,BC
        POP	BC
        DJNZ	J13B1
        RET

I13C3:	DEFM	'color ',0
        DEFM	'auto ',0
        DEFM	'goto ',0
        DEFM	'list ',0
        DEFM	'run',13,0

        IF BASVER EQ 0
        DEFM	'color 15,4,7',13,0
        ELSE
        DEFM	'color 15,4,4',13,0
        ENDIF

        DEFM	'cload"',0
        DEFM	'cont',13,0
        DEFM	'list.',13,30,30,0
        DEFM	12,'run',13,0

;	Subroutine	RDVDP
;	Inputs		________________________
;	Outputs		________________________


J140B:	IN	A,(89H)
        RET

;	Subroutine	RSLREG
;	Inputs		________________________
;	Outputs		________________________


J140E:	IN	A,(0A8H)
        RET

;	Subroutine	WSLREG
;	Inputs		________________________
;	Outputs		________________________


J1411:	OUT	(0A8H),A
        RET

;	Subroutine	PHYDIO
;	Inputs		________________________
;	Outputs		________________________


J1414:	CALL	H.PHYD
        RET

;	Subroutine	FORMAT
;	Inputs		________________________
;	Outputs		________________________;


J1418:	CALL	H.FORM
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C141C:	OUTI
        NOP
        JR	NZ,C141C
        DEC	A
        JR	NZ,C141C
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1425:	INI
        NOP
        JR	NZ,C1425
        DEC	A
        JR	NZ,C1425
        RET

J142E:	POP	AF
        PUSH	AF
        AND	0CH			; secundairy slot 0 ?
        JP	Z,J02FA			; yep, continue normally
        LD	A,H
        AND	0C0H			; page 0 ?
        JP	Z,J01B6			; yep, use special routine
        JP	J02FA			; continue normally

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C143E:	RRCA
        RRCA
        LD	L,A
        AND	0C0H			;
        LD	D,A
        LD	A,L
        AND	03H
        RET

C1448:	LD	B,A
        AND	83H
        LD	C,A
        LD	A,(EXPTBL+0)
        CP	C
        LD	A,B
        RET

C1452:	LD	A,L
C1453:	LD	L,A
        AND	3FH
        LD	B,A
        AND	03H
        RRCA
        RRCA
        OR	B
        RET

J145D:	LD	A,L
        AND	0F3H
        LD	B,A
        RLCA
        RLCA
        AND	0CH
        OR	B
        JP	J026A


        DEFS	1479H-$,0

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1479:	IN	A,(0BAH)
        AND	10H	; 16
        JR	NZ,J148C
        LD	HL,XSAVE+1
        SET	7,(HL)
        LD	A,20H	; " "
        OUT	(0BBH),A
        OR	08H	; 8
        OUT	(0BBH),A
J148C:	IN	A,(89H)
        AND	A
        RET


;	Subroutine	PUTQ
;	Inputs		________________________
;	Outputs		________________________


J1490:	CALL	C14F8
        LD	A,B
        INC	A
        INC	HL
        AND	(HL)
        CP	C
        RET	Z
        PUSH	HL
        DEC	HL
        DEC	HL
        DEC	HL
        EX	(SP),HL
        INC	HL
        LD	C,A
        LD	A,(HL)
        INC	HL
        LD	H,(HL)
        LD	L,A
        LD	B,00H
        ADD	HL,BC
        LD	(HL),E
        POP	HL
        LD	(HL),C
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C14AB:	CALL	C14F8
        LD	(HL),00H
        JR	NZ,J14CF
        LD	A,C
        CP	B
        RET	Z
        INC	HL
        INC	A
        AND	(HL)
        DEC	HL
        DEC	HL
        PUSH	HL
        INC	HL
        INC	HL
        INC	HL
        LD	C,A
        LD	A,(HL)
        INC	HL
        LD	H,(HL)
        LD	L,A
        LD	B,00H
        ADD	HL,BC
        LD	A,(HL)
        POP	HL
        LD	(HL),C
        OR	A
        RET	NZ
        INC	A
        LD	A,00H
        RET

J14CF:	LD	C,A
        LD	B,00H
        LD	HL,QUEBAK-1
        ADD	HL,BC
        LD	A,(HL)
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C14D8:	PUSH	BC
        CALL	C1502
        LD	(HL),B
        INC	HL
        LD	(HL),B
        INC	HL
        LD	(HL),B
        INC	HL
        POP	AF
        LD	(HL),A
        INC	HL
        LD	(HL),E
        INC	HL
        LD	(HL),D
        RET

;	Subroutine	LFTQ
;	Inputs		________________________
;	Outputs		________________________


J14E9:	CALL	C14F8
        LD	A,B
        INC	A
        INC	HL
        AND	(HL)
        LD	B,A
        LD	A,C
        SUB	B
        AND	(HL)
        LD	L,A
        LD	H,00H
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C14F8:	CALL	C1502
        LD	B,(HL)
        INC	HL
        LD	C,(HL)
        INC	HL
        LD	A,(HL)
        OR	A
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1502:	RLCA
        LD	B,A
        RLCA
        ADD	A,B
        LD	C,A
        LD	B,00H
        LD	HL,(QUEUES)
        ADD	HL,BC
        RET

;	Subroutine	GRPPRT
;	Inputs		________________________
;	Outputs		________________________


J150E:	CALL	C06A9
        JR	C,J151C
        PUSH	IX
        LD	IX,S.GRPPRT
        JP	J0295

J151C:	PUSH	HL
        PUSH	DE
        PUSH	BC
        PUSH	AF
        CALL	C08FA
        JR	NC,J1587
        JR	NZ,J152F
        CP	0DH	; 13
        JR	Z,J158A
        CP	20H	; " "
        JR	C,J1587
J152F:	CALL	C07AB
        LD	A,(FORCLR)
        LD	(ATRBYT),A
        LD	HL,(GRPACY)
        EX	DE,HL
        LD	BC,(GRPACX)
        CALL	C15A5
        JR	NC,J1587
        CALL	C1604
        LD	DE,PATWRK
        LD	C,08H	; 8
J154D:	LD	B,08H	; 8
        CALL	C1651
        PUSH	HL
        PUSH	AF
        LD	A,(DE)
J1555:	ADD	A,A
        PUSH	AF
        CALL	C,C16C3
        CALL	C1735
        POP	HL
        JR	C,J1564
        PUSH	HL
        POP	AF
        DJNZ	J1555
J1564:	POP	AF
        POP	HL
        CALL	C1658
        CALL	C17B4
        JR	C,J1572
        INC	DE
        DEC	C
        JR	NZ,J154D
J1572:	CALL	C15FB
        LD	A,(GRPACX)
        JR	Z,J1580
        ADD	A,20H	; " "
        JR	C,J158A
        JR	J1584

J1580:	ADD	A,08H	; 8
        JR	C,J158A
J1584:	LD	(GRPACX),A
J1587:	JP	J0937

J158A:	XOR	A
        LD	(GRPACX),A
        CALL	C15FB
        LD	A,(GRPACY)
        JR	Z,J1599
        ADD	A,20H	; " "
        DEFB	001H			; skip next instruction
J1599:	ADD	A,08H
        CP	0C0H
        JR	C,J15A0
        XOR	A
J15A0:	LD	(GRPACY),A
        JR	J1587

;	Subroutine	SCALXY
;	Inputs		________________________
;	Outputs		________________________


C15A5:	PUSH	HL
        PUSH	BC
        LD	B,01H	; 1
        EX	DE,HL
        LD	A,H
        ADD	A,A
        JR	NC,J15B3
        LD	HL,0
        JR	J15C4

J15B3:	LD	DE,192
        LD	A,(SCRMOD)
        CP	05H	; 5
        JR	C,J15BF
        LD	E,0D4H
J15BF:	RST	20H
        JR	C,J15C6
        EX	DE,HL
        DEC	HL
J15C4:	LD	B,00H
J15C6:	EX	(SP),HL
        LD	A,H
        ADD	A,A
        JR	NC,J15D0
        LD	HL,0
        JR	J15E3

J15D0:	LD	DE,256
        LD	A,(SCRMOD)
        AND	07H	; 7
        CP	06H	; 6
        JR	C,J15DE
        LD	D,02H	; 2
J15DE:	RST	20H
        JR	C,J15E5
        EX	DE,HL
        DEC	HL
J15E3:	LD	B,00H
J15E5:	POP	DE
        LD	A,(SCRMOD)
        CP	03H	; 3
        JR	NZ,J15F5
        SRL	L
        SRL	L
        SRL	E
        SRL	E
J15F5:	LD	A,B
        RRCA
        LD	B,H
        LD	C,L
        POP	HL
        RET

;	Subroutine	check if screenmode 2 or 4
;	Inputs		________________________
;	Outputs		________________________


C15FB:	LD	A,(SCRMOD)
        CP	04H	; 4
        RET	Z
        CP	02H	; 2
        RET

;	Subroutine	MAPXYC
;	Inputs		________________________
;	Outputs		________________________


C1604:	LD	A,(SCRMOD)
        CP	05H	; 5
        JR	NC,J163E
        CP	03H	; 3
        JR	Z,J1648
        PUSH	BC
        LD	D,C
        LD	A,C
        AND	07H	; 7
        LD	C,A
        LD	HL,I1636
        ADD	HL,BC
        LD	A,(HL)
        LD	(CMASK),A
        LD	A,E
        RRCA
        RRCA
        RRCA
        AND	1FH
        LD	B,A
        LD	A,D
        AND	0F8H
        LD	C,A
        LD	A,E
        AND	07H	; 7
        OR	C
        LD	C,A
        LD	HL,(GRPCGP)
        ADD	HL,BC
        LD	(CLOC),HL
        POP	BC
        RET

I1636:	DEFB	080H,040H,020H,010H,008H,004H,002H,001H

J163E:	LD	H,B
        LD	L,C
        LD	(CLOC),HL
        LD	A,E
        LD	(CMASK),A
        RET

J1648:	PUSH	IX
J164A:	LD	IX,S.MAPXYC
        JP	J0295

;	Subroutine	FETCHC
;	Inputs		________________________
;	Outputs		________________________


C1651:	LD	A,(CMASK)
        LD	HL,(CLOC)
        RET

;	Subroutine	STOREC
;	Inputs		________________________
;	Outputs		________________________


C1658:	LD	(CMASK),A
        LD	(CLOC),HL
        RET

;	Subroutine	READC
;	Inputs		________________________
;	Outputs		________________________


C165F:	CALL	C06A9
        JR	C,J166D
J1664:	PUSH	IX
        LD	IX,S.READC
        JP	J0295

J166D:	CALL	C15FB
        JR	NZ,J1664
        PUSH	BC
        PUSH	HL
        CALL	C1651
        LD	B,A
        CALL	C07E1
        AND	B
        PUSH	AF
        LD	BC,2000H
        ADD	HL,BC
        CALL	C07E1
        LD	B,A
        POP	AF
        LD	A,B
        JR	Z,J168D
        RRCA
        RRCA
        RRCA
        RRCA
J168D:	AND	0FH	; 15
        POP	HL
        POP	BC
        RET

;	Subroutine	SETATR
;	Inputs		________________________
;	Outputs		________________________


J1692:	CALL	C169A
        RET	C
        LD	(ATRBYT),A
        RET

;	Subroutine	validate graphical attribute
;	Inputs		A = attribute
;	Outputs		Cx = if error

C169A:	PUSH	AF
        LD	A,(SCRMOD)
        CP	06H	; 6
        JR	Z,J16AE
        CP	08H	; 8
        JR	Z,J16AB
        POP	AF
        CP	10H	; 16
        CCF
        RET

J16AB:	POP	AF
        AND	A
        RET

J16AE:	POP	AF
        CP	20H	; " "
        CCF
        RET	C
        CP	10H	; 16
        JR	C,J16BA
        AND	0FH	; 15
        RET

J16BA:	AND	03H	; 3
        PUSH	BC
        LD	B,A
        ADD	A,A
        ADD	A,A
        ADD	A,B
        POP	BC
        RET

;	Subroutine	SETC
;	Inputs		________________________
;	Outputs		________________________


C16C3:	LD	A,(SCRMOD)
        CP	05H	; 5
        JR	NC,J16E7
        CP	03H	; 3
        JR	Z,J16DE
        PUSH	HL
        PUSH	BC
        PUSH	DE
        LD	A,(CMASK)
        LD	HL,(CLOC)
        CALL	C18A6
        POP	DE
        POP	BC
        POP	HL
        RET

J16DE:	PUSH	IX
        LD	IX,S.SETC
        JP	J0295

J16E7:	LD	HL,(CLOC)
        LD	A,(CMASK)
        PUSH	AF
J16EE:	DI
        LD	A,02H	; 2
        OUT	(89H),A
        LD	A,8FH
        OUT	(89H),A
        PUSH	HL
        POP	HL
        IN	A,(89H)
        PUSH	AF
        XOR	A
        OUT	(89H),A
        LD	A,8FH
        OUT	(89H),A
        POP	AF
        EI
        RRCA
        JR	C,J16EE
        DI
        LD	A,24H	; "$"
        OUT	(89H),A
        LD	A,91H
        OUT	(89H),A
        LD	A,L
        OUT	(8BH),A
        LD	A,H
        OUT	(8BH),A
        POP	AF
        OUT	(8BH),A
        LD	A,(ACPAGE)
        OUT	(8BH),A
        LD	A,2CH	; ","
        OUT	(89H),A
        LD	A,91H
        OUT	(89H),A
        LD	A,(ATRBYT)
        OUT	(8BH),A
        XOR	A
        OUT	(8BH),A
        LD	A,50H	; "P"
        OUT	(8BH),A
        EI
        RET

;	Subroutine	pixel pos right if possible
;	Inputs		________________________
;	Outputs		________________________


C1735:	CALL	C15FB
        JR	Z,J1743
        PUSH	IX
        LD	IX,S.TRIGHT
        JP	J0295

J1743:	PUSH	HL
        CALL	C1651
        RRCA
        JR	NC,J17AE
        LD	A,L
        AND	0F8H
        CP	0F8H
        LD	A,80H
        JR	NZ,J176B
        JP	J181C

;	Subroutine	RIGHTC
;	Inputs		________________________
;	Outputs		________________________


C1756:	CALL	C15FB
        JR	Z,J1764
        PUSH	IX
        LD	IX,S.RIGHTC
        JP	J0295

J1764:	PUSH	HL
        CALL	C1651
        RRCA
        JR	NC,J17AE
J176B:	PUSH	DE
        LD	DE,8
        JR	J17A9

;	Subroutine	pixel pos left if possible
;	Inputs		________________________
;	Outputs		________________________


C1771:	CALL	C15FB
        JR	Z,J177F
        PUSH	IX
        LD	IX,S.TLEFTC
        JP	J0295

J177F:	PUSH	HL
        CALL	C1651
        RLCA
        JR	NC,J17AE
        LD	A,L
        AND	0F8H
        LD	A,01H	; 1
        JR	NZ,J17A5
        JP	J181C

;	Subroutine	LEFTC
;	Inputs		________________________
;	Outputs		________________________


J1790:	CALL	C15FB
        JR	Z,J179E
        PUSH	IX
        LD	IX,S.LEFTC
        JP	J0295

J179E:	PUSH	HL
        CALL	C1651
        RLCA
        JR	NC,J17AE
J17A5:	PUSH	DE
        LD	DE,-8
J17A9:	ADD	HL,DE
        LD	(CLOC),HL
        POP	DE
J17AE:	LD	(CMASK),A
        AND	A
        POP	HL
        RET

;	Subroutine	TDOWNC
;	Inputs		________________________
;	Outputs		________________________


C17B4:	CALL	C15FB
        JR	Z,J17C2
        PUSH	IX
        LD	IX,S.TDOWNC
        JP	J0295

J17C2:	PUSH	HL
        PUSH	DE
        LD	HL,(CLOC)
        PUSH	HL
        LD	HL,(GRPCGP)
        LD	DE,1700H
        ADD	HL,DE
        EX	DE,HL
        POP	HL
        RST	20H
        JR	C,J17EF
        LD	A,L
        INC	A
        AND	07H	; 7
        JR	NZ,J17EF
        JR	J181B

;	Subroutine	DOWNC
;	Inputs		________________________
;	Outputs		________________________


J17DC:	CALL	C15FB
        JR	Z,J17EA
        PUSH	IX
        LD	IX,S.DOWNC
        JP	J0295

J17EA:	PUSH	HL
        PUSH	DE
        LD	HL,(CLOC)
J17EF:	INC	HL
        LD	A,L
        LD	DE,00F8H
        JR	J1837

;	Subroutine	TUPC
;	Inputs		________________________
;	Outputs		________________________


J17F6:	CALL	C15FB
        JR	Z,J1804
        PUSH	IX
        LD	IX,S.TUPC
        JP	J0295

J1804:	PUSH	HL
        PUSH	DE
        LD	HL,(CLOC)
        PUSH	HL
        LD	HL,(GRPCGP)
        LD	DE,256
        ADD	HL,DE
        EX	DE,HL
        POP	HL
        RST	20H
        JR	NC,J1832
        LD	A,L
        AND	07H	; 7
        JR	NZ,J1832
J181B:	POP	DE
J181C:	SCF
        POP	HL
        RET

;	Subroutine	UPC
;	Inputs		________________________
;	Outputs		________________________


J181F:	CALL	C15FB
        JR	Z,J182D
        PUSH	IX
        LD	IX,S.UPC
        JP	J0295

J182D:	PUSH	HL
        PUSH	DE
        LD	HL,(CLOC)
J1832:	LD	A,L
        DEC	HL
        LD	DE,0FF08H
J1837:	AND	07H	; 7
        JR	NZ,J183C
        ADD	HL,DE
J183C:	LD	(CLOC),HL
        AND	A
        POP	DE
        POP	HL
        RET

;	Subroutine	NSETCX
;	Inputs		________________________
;	Outputs		________________________


C1843:	CALL	C15FB
        JP	NZ,J18F5
        PUSH	HL
        CALL	C1651
        EX	(SP),HL
        ADD	A,A
        JR	C,J1869
        PUSH	AF
        LD	BC,D.FFFF
        RRCA
J1856:	ADD	HL,BC
        JR	NC,J189E
        RRCA
        JR	NC,J1856
        POP	AF
        DEC	A
        EX	(SP),HL
        PUSH	HL
        CALL	C18A6
        POP	HL
        LD	DE,8
        ADD	HL,DE
        EX	(SP),HL
J1869:	LD	A,L
        AND	07H	; 7
        LD	C,A
        LD	A,H
        RRCA
        LD	A,L
        RRA
        RRCA
        RRCA
        AND	3FH	; "?"
        POP	HL
        LD	B,A
        JR	Z,J188D
J1879:	XOR	A
        CALL	C07D7
        LD	DE,2000H
        ADD	HL,DE
        LD	A,(ATRBYT)
        CALL	C07D7
        LD	DE,2008H
        ADD	HL,DE
        DJNZ	J1879
J188D:	DEC	C
        RET	M
        PUSH	HL
        LD	HL,I1897
        ADD	HL,BC
        LD	A,(HL)
        JR	J18A5

I1897:	DEFB	080H,0C0H,0E0H,0F0H,0F8H,0FCH,0FEH

J189E:	ADD	A,A
        DEC	A
        CPL
        LD	B,A
        POP	AF
        DEC	A
        AND	B
J18A5:	POP	HL


;	Subroutine	__________________________
;	Inputs		________________________
;	Outputs		________________________


C18A6:	LD	B,A
        CALL	C07E1
        LD	C,A
        LD	DE,2000H
        ADD	HL,DE
        CALL	C07E1
        PUSH	AF
        AND	0FH	; 15
        LD	E,A
        POP	AF
        SUB	E
        LD	D,A
        LD	A,(ATRBYT)
        CP	E
        JR	Z,J18D8
        ADD	A,A
        ADD	A,A
        ADD	A,A
        ADD	A,A
        CP	D
        JR	Z,C18DC
        PUSH	AF
        LD	A,B
        OR	C
        CP	0FFH
        JR	Z,J18E4
        PUSH	HL
        PUSH	DE
        CALL	C18DC
        POP	DE
        POP	HL
        POP	AF
        OR	E
        JR	J18F2

J18D8:	LD	A,B
        CPL
        AND	C
        DEFB	011H			; LD DE,xxxx (skip next 2 instructions)
C18DC:	LD	A,B
        OR	C


;	Subroutine	__________________________
;	Inputs		________________________
;	Outputs		________________________


C18DE:	LD	DE,2000H
        ADD	HL,DE
        JR	J18F2

J18E4:	POP	AF
        LD	A,B
        CPL
        PUSH	HL
        PUSH	DE
        CALL	C18DE
        POP	DE
        POP	HL
        LD	A,(ATRBYT)
        OR	D
J18F2:	JP	C07D7

J18F5:	PUSH	HL
        CALL	C16C3
        CALL	C1756
        POP	HL
        DEC	L
        JR	NZ,J18F5
        RET

;	Subroutine	GTASPC
;	Inputs		________________________
;	Outputs		________________________


J1901:	LD	HL,(ASPCT1)
        EX	DE,HL
        LD	HL,(ASPCT2)
        RET

;	Subroutine	PNTINI
;	Inputs		________________________
;	Outputs		________________________


J1909:	PUSH	AF
        CALL	C15FB
        JR	Z,J1915
        POP	AF
        CP	10H	; 16
        CCF
        JR	J191A

J1915:	POP	AF
        LD	A,(ATRBYT)
        AND	A
J191A:	LD	(BDRATR),A
        RET

;	Subroutine	SCANR
;	Inputs		________________________
;	Outputs		________________________


J191E:	LD	HL,0
        LD	C,L
        CALL	C15FB
        JR	Z,J1930
        PUSH	IX
        LD	IX,S.SCANR
        JP	J0295

J1930:	LD	A,B
        LD	(FILNAM+0),A
        XOR	A
        LD	(FILNAM+3),A
        LD	A,(BDRATR)
        LD	B,A
J193C:	CALL	C165F
        CP	B
        JR	NZ,J194F
        DEC	DE
        LD	A,D
        OR	E
        RET	Z
        CALL	C1735
        JR	NC,J193C
        LD	DE,0
        RET

J194F:	CALL	C19D1
        PUSH	DE
        CALL	C1651
        LD	(CSAVEA),HL
        LD	(CSAVEM),A
        LD	DE,0
J195F:	INC	DE
        CALL	C1735
        JR	C,J1970
        CALL	C165F
        CP	B
        JR	Z,J1970
        CALL	C19D1
        JR	J195F

J1970:	PUSH	DE
        CALL	C1651
        PUSH	HL
        PUSH	AF
        LD	HL,(CSAVEA)
        LD	A,(CSAVEM)
        CALL	C1658
        EX	DE,HL
        LD	(FILNAM+1),HL
        LD	A,(FILNAM+0)
        AND	A
        CALL	NZ,C1843
        POP	AF
        POP	HL
        CALL	C1658
        POP	HL
        POP	DE
        JP	J19CC

;	Subroutine	SCANL
;	Inputs		________________________
;	Outputs		________________________


J1994:	LD	HL,0
        LD	C,L
        CALL	C15FB
        JR	Z,J19A6
        PUSH	IX
        LD	IX,S.SCANL
        JP	J0295

J19A6:	XOR	A
        LD	(FILNAM+3),A
        LD	A,(BDRATR)
        LD	B,A
J19AE:	CALL	C1771
        JR	C,J19C2
        CALL	C165F
        CP	B
        JR	Z,J19BF
        CALL	C19D1
        INC	HL
        JR	J19AE

J19BF:	CALL	C1756
J19C2:	PUSH	HL
        LD	DE,(FILNAM+1)
        ADD	HL,DE
        CALL	C1843
        POP	HL
J19CC:	LD	A,(FILNAM+3)
        LD	C,A
        RET


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C19D1:	PUSH	HL
        LD	HL,ATRBYT
        CP	(HL)
        POP	HL
        RET	Z
        INC	A
        LD	(FILNAM+3),A
        RET

J19DD:	PUSH	BC
        PUSH	AF
        LD	BC,0
J19E2:	DEC	BC
        LD	A,B
        OR	C
        JR	NZ,J19E2
        POP	AF
        POP	BC
J19E9:	PUSH	AF
        LD	A,09H	; 9
        OUT	(0ABH),A
        POP	AF
        EI
        RET

J19F1:	OR	A
        PUSH	AF
        LD	A,08H	; 8
        OUT	(0ABH),A
        LD	HL,0
J19FA:	DEC	HL
        LD	A,H
        OR	L
        JR	NZ,J19FA
        POP	AF
        LD	A,(HEADER)
        JR	Z,J1A07
        ADD	A,A
        ADD	A,A
J1A07:	LD	B,A
        LD	C,00H
        DI
J1A0B:	CALL	C1A4D
        CALL	C1A3F
        DEC	BC
        LD	A,B
        OR	C
        JR	NZ,J1A0B
        JP	C0564
J1A19:	LD	HL,(LOW.)
        PUSH	AF
        LD	A,L
        SUB	0EH	; 14
        LD	L,A
        CALL	C1A50
        POP	AF
        LD	B,08H	; 8
J1A27:	RRCA
        CALL	C,C1A40
        CALL	NC,C1A39
        DJNZ	J1A27
        CALL	C1A40
        CALL	C1A40
        JP	C0564

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1A39:	LD	HL,(LOW.)
        CALL	C1A50

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1A3F:	RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1A40:	CALL	C1A4D
        EX	(SP),HL
        EX	(SP),HL
        DEFB	0,0,0,0
        CALL	C1A4D
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1A4D:	LD	HL,(HIGH.)

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1A50:	PUSH	AF
J1A51:	DEC	L
        JP	NZ,J1A51
        LD	A,0BH	; 11
        OUT	(0ABH),A
J1A59:	DEC	H
        JP	NZ,J1A59
        LD	A,0AH	; 10
        OUT	(0ABH),A
        POP	AF
        RET

J1A63:	LD	A,08H	; 8
        OUT	(0ABH),A
        DI
        LD	A,0EH	; 14
        OUT	(0A0H),A
J1A6C:	LD	HL,0457H
J1A6F:	LD	D,C
        CALL	C1B34
        RET	C
        LD	A,C
        CP	0DEH
        JR	NC,J1A6C
        CP	05H	; 5
        JR	C,J1A6C
        SUB	D
        JR	NC,J1A82
        CPL
        INC	A
J1A82:	CP	04H	; 4
        JR	NC,J1A6C
        DEC	HL
        LD	A,H
        OR	L
        JR	NZ,J1A6F
        LD	HL,0
        LD	B,L
        LD	D,L
J1A90:	CALL	C1B34
        RET	C
        ADD	HL,BC
        DEC	D
        JP	NZ,J1A90
        LD	BC,06AEH
        ADD	HL,BC
        LD	A,H
        RRA
        AND	7FH
        LD	D,A
        ADD	HL,HL
        LD	A,H
        SUB	D
        LD	D,A
        SUB	06H	; 6
        LD	(LOWLIM),A
        LD	A,D
        ADD	A,A
        LD	B,00H
J1AAF:	SUB	03H	; 3
        INC	B
        JR	NC,J1AAF
        LD	A,B
        SUB	03H	; 3
        LD	(WINWID),A
        OR	A
        RET

J1ABC:	LD	A,(LOWLIM)
        LD	D,A
J1AC0:	CALL	C0564
        RET	C
        IN	A,(0A2H)
        RLCA
        JR	NC,J1AC0
J1AC9:	CALL	C0564
        RET	C
        IN	A,(0A2H)
        RLCA
        JR	C,J1AC9
        LD	E,00H
        CALL	C1B1F
J1AD7:	LD	B,C
        CALL	C1B1F
        RET	C
        LD	A,B
        ADD	A,C
        JP	C,J1AD7
        CP	D
        JR	C,J1AD7
        LD	L,08H	; 8
J1AE6:	CALL	C1B03
        CP	04H	; 4
        CCF
        RET	C
        CP	02H	; 2
        CCF
        RR	D
        LD	A,C
        RRCA
        CALL	NC,C1B23
        CALL	C1B1F
        DEC	L
        JP	NZ,J1AE6
        CALL	C0564
        LD	A,D
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1B03:	LD	A,(WINWID)
        LD	B,A
        LD	C,00H
J1B09:	IN	A,(0A2H)
        XOR	E
        JP	P,J1B17
        LD	A,E
        CPL
        LD	E,A
        INC	C
        DJNZ	J1B09
        LD	A,C
        RET
J1B17:	DEFB	0,0,0,0
J1B1B:	DJNZ	J1B09
        LD	A,C
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1B1F:	CALL	C0564
        RET	C

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1B23:	LD	C,00H
J1B25:	INC	C
        JR	Z,J1B32
        IN	A,(0A2H)
        XOR	E
        JP	P,J1B25
        LD	A,E
        CPL
        LD	E,A
        RET
J1B32:	DEC	C
        RET

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C1B34:	CALL	C0564
        RET	C
        IN	A,(0A2H)
        RLCA
        JR	C,C1B34
        LD	E,00H
        CALL	C1B23
        JP	J1B25

;	Subroutine	OUTDO
;	Inputs		________________________
;	Outputs		________________________


J1B45:	PUSH	AF
        CALL	H.OUTD
        CALL	C0BF9
        JR	Z,J1B56
        POP	AF
        LD	IX,M6C48
        JP	C02BF

J1B56:	LD	A,(PRTFLG)
        OR	A
        JR	Z,J1BBB
        LD	A,(RAWPRT)
        AND	A
        JR	NZ,J1BAB
        POP	AF

;	Subroutine	OUTDLP
;	Inputs		________________________
;	Outputs		________________________


C1B63:	PUSH	AF
        CP	09H	; 9
        JR	NZ,J1B76
J1B68:	LD	A,20H	; " "
        CALL	C1B63
        LD	A,(LPTPOS)
        AND	07H	; 7
        JR	NZ,J1B68
        POP	AF
        RET

J1B76:	SUB	0DH	; 13
        JR	Z,J1B84
        JR	C,J1B87
        CP	13H	; 19
        JR	C,J1B87
        LD	A,(LPTPOS)
        INC	A
J1B84:	LD	(LPTPOS),A
J1B87:	LD	A,(NTMSXP)
        AND	A
        JR	Z,J1BAB
        POP	AF
        CALL	C08FA
        RET	NC
        JR	NZ,J1BB7


        IF	KEYTYP EQ 0

; *************************************
; *************************************


        AND	A
        JP	P,J1BAC			; plain ascii, print
        CP	86H
        JR	C,J1BB7			; 80H-85H, print space
        CP	0A0H			; 86H-9FH ?
        JR	NC,J1BA4		; nope,
        ADD	A,20H
        JR	J1BAC			; adjust to A6H-BFH and print

J1BA4:	CP	0E0H			; E0H-FFH ?
        JR	C,J1BAC			; nope, print
        SUB	20H			; adjust to C0H-DFH
        DEFB	038H			; JR C,xxxx (skip next instruction)


; *************************************
; *************************************

        ENDIF

        ORG	01BABH

J1BAB:	POP	AF
J1BAC:	CALL	C08BA
        RET	NC
        LD	IX,M73B2
        JP	C02BF

J1BB7:	LD	A,20H	; " "
        JR	J1BAC

J1BBB:	POP	AF
        JP	J0919
;	-----------------
;	Characterset goes here
;	BEGIN
;	-----------------


D1BBF:

        IF	CHRGEN EQ 0

        IF	CNTRY EQ 9
        INCLUDE	CHRKOR
        ELSE
        INCLUDE CHRJAP
        ENDIF

        ENDIF

        IF	CHRGEN EQ 1

        IF	CNTRY EQ 10
        INCLUDE	CHRRUS
        ENDIF
        IF	CNTRY EQ 5
        INCLUDE CHRGER
        ENDIF

        IF	(CNTRY NE 10) AND (CNTRY NE 5)
        INCLUDE CHRINT
        ENDIF

        ENDIF


;	-----------------
;	Characterset goes here
;	END
;	-----------------

;	Subroutine	PINLIN
;	Inputs		________________________
;	Outputs		________________________


J23BF:	CALL	H.PINL
        LD	A,(AUTFLG)
        AND	A
        JR	NZ,J23D5
        LD	L,00H
        JR	J23E0

;	Subroutine	QINLIN
;	Inputs		________________________
;	Outputs		________________________


J23CC:	CALL	H.QINL
        LD	A,3FH	; "?"
        RST	18H
        LD	A,20H	; " "
        RST	18H

;	Subroutine	INLIN
;	Inputs		________________________
;	Outputs		________________________


J23D5:	CALL	H.INLI
        LD	HL,(CSRY)
        DEC	L
        CALL	NZ,C0BD9
        INC	L
J23E0:	LD	(FSTPOS),HL
        XOR	A
        LD	(INTFLG),A
J23E7:	CALL	C10CB
        LD	HL,I2439-2
        LD	C,0BH	; 11
        CALL	C0976
        PUSH	AF
        CALL	NZ,C23FF
        POP	AF
        JR	NC,J23E7
        LD	HL,BUFMIN
        RET	Z
        CCF

; escape (do nothing)

C23FE:	RET


;	Subroutine	__________________________
;	Inputs		________________________
;	Outputs		________________________


C23FF:	PUSH	AF
        CP	09H	; 9
        JR	NZ,J2413
        POP	AF
J2405:	LD	A,20H	; " "
        CALL	C23FF
        LD	A,(CSRX)
        DEC	A
        AND	07H	; 7
        JR	NZ,J2405
        RET

J2413:	POP	AF
        LD	HL,INSFLG
        CP	01H	; 1
        JR	Z,J2426
        CP	20H	; " "
        JR	C,J2428
        PUSH	AF
        LD	A,(HL)
        AND	A
        CALL	NZ,C24F2
        POP	AF
J2426:	RST	18H
        RET

J2428:	LD	(HL),00H
        RST	18H
        DEFB	03EH			; LD A,xx (skip next instruction)

;	Subroutine	change to "insert" cursor
;	Inputs		________________________
;	Outputs		________________________


J242C:	DEFB	03EH			; LD A,xx (skip next instruction)

;	Subroutine	change to "block" cursor
;	Inputs		________________________
;	Outputs		________________________


C242D:	XOR	A
        PUSH	AF
        CALL	C0A8B
        POP	AF
        LD	(CSTYLE),A
        JP	C0A3E


I2439:	DEFB	8
        DEFW	C2561			; backspace
        DEFB	18
        DEFW	C24E5			; insert
        DEFB	27
        DEFW	C23FE			; escape
        DEFB	2
        DEFW	C260E			; ctrl-b (move to start of previous word)
        DEFB	6
        DEFW	C25F8			; ctrl-f (move to start of next word)
        DEFB	14
        DEFW	C25D7			; ctrl-n (append at end of line)
        DEFB	5
        DEFW	C25B9			; ctrl-e (erase text to the end of logical line)
        DEFB	3
        DEFW	C24C5			; ctrl-stop/ctrl-c
        DEFB	13
        DEFW	C245A			; return
        DEFB	21
        DEFW	C25AE			; ctrl-u (erase logical line)
        DEFB	127
        DEFW	C2550			; delete

; return

C245A:	CALL	C266C
        LD	A,(AUTFLG)
        AND	A
        JR	Z,J2465
        LD	H,01H	; 1
J2465:	PUSH	HL
        CALL	C0A8B
        POP	HL
        LD	DE,BUF
        LD	B,0FEH
        DEC	L
J2470:	INC	L
J2471:	PUSH	DE
        PUSH	BC
        CALL	C0B83
        POP	BC
        POP	DE
        AND	A
        JR	Z,J248F
        CP	20H	; " "
        JR	NC,J248A
        DEC	B
        JR	Z,J249F
        LD	C,A
        LD	A,01H	; 1
        LD	(DE),A
        INC	DE
        LD	A,C
        ADD	A,40H	; "@"
J248A:	LD	(DE),A
        INC	DE
        DEC	B
        JR	Z,J249F
J248F:	INC	H
        LD	A,(LINLEN)
        CP	H
        JR	NC,J2471
        PUSH	DE
        CALL	C0BCD
        POP	DE
        LD	H,01H	; 1
        JR	Z,J2470
J249F:	DEC	DE
        LD	A,(DE)
        CP	20H	; " "
        JR	Z,J249F
        PUSH	HL
        PUSH	DE
        CALL	C0A3E
        POP	DE
        POP	HL
        INC	DE
        XOR	A
        LD	(DE),A
J24AF:	LD	A,0DH	; 13
        AND	A
J24B2:	PUSH	AF
        CALL	C0BD9
        CALL	C08EB
        LD	A,0AH	; 10
        RST	18H
        XOR	A
        LD	(INSFLG),A
        POP	AF
        SCF
        POP	HL
        RET

J24C4:	INC	L

; ctrl-stop/ctrl-c

C24C5:	CALL	C0BCD
        JR	Z,J24C4
        CALL	C242D
        XOR	A
        LD	(BUF+0),A
        LD	H,01H	; 1
        PUSH	HL
        CALL	C05B2
        CALL	C0549
        POP	HL
        JR	C,J24AF
        LD	A,(BASROM)
        AND	A
        JR	NZ,J24AF
        JR	J24B2

; insert

C24E5:	LD	HL,INSFLG
        LD	A,(HL)
        XOR	0FFH
        LD	(HL),A
        JP	Z,C242D
        JP	J242C


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C24F2:	CALL	C0A8B
        LD	HL,(CSRY)
        LD	C,20H	; " "
J24FA:	PUSH	HL
J24FB:	PUSH	BC
        CALL	C0B83
        POP	DE
        PUSH	BC
        LD	C,E
        CALL	C0B8D
        POP	BC
        LD	A,(LINLEN)
        INC	H
        CP	H
        LD	A,D
        JR	NC,J24FB
        POP	HL
        CALL	C0BCD
        JR	Z,J254B
        LD	A,C
        CP	20H	; " "
        PUSH	AF
        JR	NZ,J2524
        LD	A,(LINLEN)
        CP	H
        JR	Z,J2524
        POP	AF
        JP	C0A3E

J2524:	CALL	C0BDA
        INC	L
        PUSH	BC
        PUSH	HL
        CALL	C0BE2
        CP	L
        JR	C,J2535
        CALL	C0AF1
        JR	J2544

J2535:	LD	HL,CSRY
        DEC	(HL)
        JR	NZ,J253C
        INC	(HL)
J253C:	LD	L,01H	; 1
        CALL	C0AE5
        POP	HL
        DEC	L
        PUSH	HL
J2544:	POP	HL
        POP	BC
        POP	AF
        JP	Z,C0A3E
        DEC	L
J254B:	INC	L
        LD	H,01H	; 1
        JR	J24FA

; delete

C2550:	LD	A,(LINLEN)
        CP	H
        JR	NZ,J255B
        CALL	C0BCD
        JR	NZ,J2595
J255B:	LD	A,1CH
        RST	18H
        LD	HL,(CSRY)

; backspace

C2561:	PUSH	HL
        CALL	C0A8B
        POP	HL
        DEC	H
        JP	NZ,J257A
        INC	H
        PUSH	HL
        DEC	L
        JR	Z,J2579
        LD	A,(LINLEN)
        LD	H,A
        CALL	C0BCD
        JR	NZ,J2579
        EX	(SP),HL
J2579:	POP	HL
J257A:	LD	(CSRY),HL
J257D:	LD	A,(LINLEN)
        CP	H
        JR	Z,J2595
        INC	H
J2584:	CALL	C0B83
        DEC	H
        CALL	C0B8D
        INC	H
        INC	H
        LD	A,(LINLEN)
        INC	A
        CP	H
        JR	NZ,J2584
        DEC	H
J2595:	LD	C,20H	; " "
        CALL	C0B8D
        CALL	C0BCD
        JP	NZ,C0A3E
        PUSH	HL
        INC	L
        LD	H,01H	; 1
        CALL	C0B83
        EX	(SP),HL
        CALL	C0B8D
        POP	HL
        JR	J257D

; ctrl-u (erase logical line)

C25AE:	CALL	C0A8B
        CALL	C266C
        LD	(CSRY),HL
        JR	J25BE

; ctrl-e (erase text to the end of logical line)

C25B9:	PUSH	HL
        CALL	C0A8B
        POP	HL
J25BE:	CALL	C0BCD
        PUSH	AF
        CALL	C0B05
        POP	AF
        JR	NZ,J25CD
        LD	H,01H	; 1
        INC	L
        JR	J25BE

J25CD:	CALL	C0A3E
        XOR	A
        LD	(INSFLG),A
        JP	C242D

; ctrl-n (append at end of line)

C25D7:	CALL	C0A8B
        LD	HL,(CSRY)
        DEC	L
J25DE:	INC	L
        CALL	C0BCD
        JR	Z,J25DE
        LD	A,(LINLEN)
        LD	H,A
        INC	H
J25E9:	DEC	H
        JR	Z,J25F3
        CALL	C0B83
        CP	20H	; " "
        JR	Z,J25E9
J25F3:	CALL	C0AB8
        JR	J25CD

; ctrl-f (move to start of next word)

C25F8:	CALL	C0A8B
        CALL	C2634
J25FE:	CALL	C2624
        JR	Z,J25CD
        JR	C,J25FE
J2605:	CALL	C2624
        JR	Z,J25CD
        JR	NC,J2605
        JR	J25CD

; ctrl-b (move to start of previous word)

C260E:	CALL	C0A8B
J2611:	CALL	C2634
        JR	Z,J25CD
        JR	NC,J2611
J2618:	CALL	C2634
        JR	Z,J25CD
        JR	C,J2618
        CALL	C0AB8
        JR	J25CD

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C2624:	LD	HL,(CSRY)
        CALL	C0AB8
        CALL	C0BE2
        LD	E,A
        LD	A,(LINLEN)
        LD	D,A
        JR	J263D

;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________

C2634:	LD	HL,(CSRY)
        CALL	C0AA9
        LD	DE,0101H
J263D:	LD	HL,(CSRY)
        RST	20H
        RET	Z
        LD	DE,I2668
        PUSH	DE
        CALL	C0B83
        CP	30H	; "0"
        CCF
        RET	NC
        CP	3AH	; ":"
        RET	C
        CP	41H	; "A"
        CCF
        RET	NC
        CP	5BH	; "["
        RET	C
        CP	61H	; "a"
        CCF
        RET	NC
        CP	7BH	; "{"
        RET	C
        CP	86H
        CCF
        RET	NC
        CP	0A0H
        RET	C
        CP	0A6H
        CCF
I2668:	LD	A,00H
        INC	A
        RET


;	  Subroutine __________________________
;	     Inputs  ________________________
;	     Outputs ________________________


C266C:	DEC	L
        JR	Z,J2674
        CALL	C0BCD
        JR	Z,C266C
J2674:	INC	L
        LD	A,(FSTPOS)
        CP	L
        LD	H,01H	; 1
        RET	NZ
        LD	HL,(FSTPOS)
        RET


; Some sort of jumptable for routines in basicrom


; entrypoint for 2nd part of the init routine
; not used, remained for compatibility reasons

?2680:	JP	M7C76

; entrypoint for SYNCHR

J2683:	JP	M558C

; entrypoint for CHRGTR

J2686:	JP	M4666

; entrypoint for GETYPR

J2689:	JP	M5597


        END
