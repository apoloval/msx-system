...
; DOS 1.02 kernel (most common used), NEOS MA-20 version
...
A580C:	push	hl			; fake start of workarea
        jr	A57F3			; hook H.RUNC and register fake driver
...
A588F:	call	INIENV			; initialize driver workarea
        ld	hl,DEVICE
...
A589C:	ld	(hl),0C9H
        ...
        ld	hl,DEVICE
        dec	(hl)			; leave my own (fake) diskinterface out
        xor	a
        cp	(hl)
        ld	(hl),a			; clear diskinterface count
        call	A7421			; patch routine
...
A5967:	inc	hl
        ...
        ld	c,000H
        call	A750F			; search ram in page 0 (patched)
        jr	c,A59C1
...
J5A36:	LD	(HL),A
        ...
        LD	BC,379			; size of the slotswitching routines
        CALL	A5EAD			; allocate MSXDOS memory (halt when error)
        PUSH	HL
        EX	DE,HL
        LD	HL,C63F4
...
;	Inputs		-
;	Outputs		HL = pointer to EXPTBL entry
...
A6212:	inc	hl
        ...
        call	nz,RDSLT		; yep, read DSKSTP entry
        and	a			; entry for interface used AND a valid DSKSTP entry ?
        call	nz,CALSLT		; yep, call DSKSTP of interface
        pop	bc
        pop	hl
...
C64C2:	BIT	7,H			; page 0 or 1 ?
        JR	NZ,J64D5		; nope, normal method
        BIT	7,A			; expanded slot ?
        JR	NZ,J64CC		; yep,
        AND	03H			; clear unused bits
J64CC:	LD	B,A
        AND	0FH			; slot 0 or 0-0 ?
        LD	A,B
        JR	NZ,J64D5		; nope, normal methode
        LD	A,(EXPTBL+0)		; replace 0 or 0-0 by slotid MA-20 mainrom
J64D5:	DI
        PUSH	AF
        LD	A,H
        RLCA
...
;	  Subroutine replace routine for A622D (CALL at A58A8 of diskrom)
;	     Inputs  ________________________
;	     Outputs ________________________

A7421:	JP	M,A622D			; diskinterface count not cleared, continue normally
        POP	HL			; do not return to init routine
        LD	HL,VARWRK
        LD	(HIMEM),HL
        CALL	A5F5F			; setup i/o channels
        CALL	A402D			; GETSLT
        ...
        ADD	A,C
        LD	C,A
J7444:	LD	HL,SLTATR+1
        ADD	HL,BC
        LD	(HL),B			; clear expansion flags
        RET
...
J7452:	LD	A,(EXPTBL+0)
        ...
        IN	A,(0A8H)
        AND	0C0H
        CP	B			; is system RAM in the same primairy slot as the MA-20 mainrom ?
        RET	Z			; yep, quit
        LD	A,C
        CALL	C74AE			; search RAM in the same primairy slot as the MA-20 mainrom
...
J7499:	LD	A,E
        ...
        LD	A,H
        CP	0C0H
        JR	NC,J7499			; copy page 3
        LD	IX,M7B61
        JP	CALBAS				; initialize system
...
C74AE:	RRCA
        ...
        AND	3FH
        OR	B
        OUT	(0A8H),A			; select primairy slot of MA-20 mainrom in page 3
        LD	A,(D.FFFF)
        CPL
        LD	D,A
        AND	3FH				; select secundairy slot 0
J74C2:	LD	C,A
        LD	(D.FFFF),A
        LD	HL,0FFFFH-256
J74C9:	LD	A,(HL)
        ...
        CPL
        LD	(HL),A
        JR	NZ,J74E9			; nope, next secundairy slot
        DEC	HL
        LD	A,H
        CP	0C0H
        JR	NC,J74C9
...
C74F7:	LD	B,0			; primairy slot 0
J74F9:	LD	A,(EXPTBL+0)
        AND	03H
        CP	B			; slot of NEOS cartridge ?
        LD	A,B
        SCF
        PUSH	BC
        CALL	NZ,C752B		; nope, search RAM in page 0
        POP	BC
        RET	NC			; RAM found, quit
        INC	B
        LD	A,B
        CP	04H
        JR	NZ,J74F9		; next primairy slot
        SCF
        RET

;	  Subroutine replacement routine for A5E52, CALL at A59B9 replaced by CALL A750F
;	     Inputs  ________________________
;	     Outputs ________________________

A750F:	CALL	C74F7			; search RAM in page 0 other than NEOS primary slot
        RET	NC			; found, quit
        CALL	C7526			; search RAM in page 0 in NEOS primary slot
        RET	C			; not found, quit
        LD	H,A
        RRCA
        RRCA
        AND	0C0H
        LD	L,A			; slotid (page 3 based)
        IN	A,(0A8H)
        AND	0C0H
        CP	L			; slotid of systemvars same as the NEOS RAM ?
        LD	A,H
        RET	Z			; yep, ok
        SCF				; RAM 'not found'
        RET
...
A7578:	CALL	A5FE7			; get my EXPTBL entry
        LD	A,(HL)
        AND	80H			; slot expanded flag
        RET

A757F:	CALL	A5FEC			; get EXPTBL entry
        LD	A,(HL)
        AND	80H			; slot expanded flag
        RET
...
;	  Subroutine slot switching routines for 003BH
;	     Inputs  ________________________
;	     Outputs ________________________
;	     Remark  orginal routines at 655CH moved here because of patch
