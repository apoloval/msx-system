...
C7987:	LD	(NLONLY),A		; not loading basic program, close i/o channels when requested
        ...
        OUT	(VDP.CW),A
        LD	A,0AEH
        OUT	(VDP.CW),A		; cancel any vdp command in progress
        EI
        RET
; #####################
...
;	LD	(SLTTBL+2),HL			; save secundiary slotregisters
        ...
        LD	BC,EXPTBL-1-VARWRK-1
        LD	DE,VARWRK+1
        LD	HL,VARWRK
        LD	(HL),0
        LDIR
        LD	BC,0FFF8H-SLTATR-1
        LD	DE,SLTATR+1
        LD	HL,SLTATR
        LD	(HL),0
        LDIR					; clear system variables except EXPTBL and SLTTBL
        LD	A,(EXPTBL+0)
        LD	(CGPNT+0),A			; slotid characterset = slotid mainrom
...
PCRT1:
        LD	HL,SLTATR
        LD	A,080H				; assume expanded slot 0
        JP	PCRT2
...
C7D75:	DI
        ...
        LD	DE,EXPTBL
        JP	PCRT1			; patch for EXPTBL entry slot 0
J7D7E:	LD	A,(DE)
PCRT2:	OR	C
        LD	C,A
        PUSH	DE
...
T7EE4:	DEFB	"version 3.0",13,10
        DEFB	0

T7EF2:	DEFB	"MSX BASIC "
        DEFB	0
J7EFD:	DEFB	"Copyright 1988 by Microsoft",13,10
        DEFB	0
