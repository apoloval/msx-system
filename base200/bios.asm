...
; MSX BIOS ROM, MSX 2 version (version 2.0)
...
J03B9:  PUSH    HL
        LD      HL,0
        CALL    M7E1A                   ; read word from extension ROM (address 0000H, indentifier)
        PUSH    HL
        LD      HL,4443H
        RST     20H                     ; SUBROM identifier ?
        POP     HL
        LD      B,00H
        JR      NZ,J03EF                ; nope,
        CALL    M7E1A                   ; read word from extension ROM (address 0002H, initialize)
        PUSH    HL
        PUSH    BC
        PUSH    DE
        POP     IX
        ...
        PUSH    AF
        POP     IY
        CALL    NZ,C0217                ; if SUBROM has initialize routine, call it
        POP     BC
        POP     HL
        CALL    M7E1A                   ; read word from extension ROM (address 0004H, call statement handler)
        ADD     A,0FFH
        RR      B
        CALL    M7E1A                   ; read word from extension ROM (address 0006H, device handler)
        ADD     A,0FFH
        RR      B
        SRL     B
        LD      DE,-8
...
J0416:  XOR     A
        ...
        JP      PTCPAL                  ; patch routine to setup VDP for PAL (JP, because no RAM yet)
        NOP

        ENDIF

M.0431: LD      DE,0FFFFH               ; initialize lowest RAM address found (page 2)
        XOR     A                       ; start with primairy slot 0
        LD      C,A                     ; initialize expanded slot flags
J0436:  OUT     (0A8H),A                ; select primairy slot on page 2 and 3
...
J0476:  LD      A,B
        ...
        LD      A,(D.FFFF)
        CPL
        ADD     A,10H                   ; next secundairy slot page 2
        CP      40H                     ; all secundairy slots done ?
...
J0484:  IN      A,(0A8H)
        ...
        IN      A,(0A8H)
        AND     3FH                     ; primairy slot 0 in page 3
J04A2:  OUT     (0A8H),A                ; select new primairy slot in page 3
        LD      B,0
        RLC     C                       ; slot expanded ?
        JR      NC,J04B4                ; nope,
        ...
        LD      A,(D.FFFF)
        CPL
        AND     3FH                     ; secundairy slot 0 in page 3
J04B1:  LD      (D.FFFF),A              ; select new secundairy slot in page 3
J04B4:  LD      HL,0FEFFH-255           ; page 3 (leave upper 256 bytes out for secundairy slot register)
J04B7:  LD      A,(HL)
        CPL
        LD      (HL),A
        CP      (HL)                    ; RAM found (write inverted correctly read back) ?
...
J04DB:  LD      A,B
        ...
        LD      A,(D.FFFF)
        CPL
        ADD     A,40H                   ; next secundairy slot page 3
        JR      NC,J04B1                ; all secundairy slots done ? nope, next
J04E7:  IN      A,(0A8H)
