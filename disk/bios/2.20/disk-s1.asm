...
IF1D3   EQU     0F1D3H                  ; handlers
CF1FD   EQU     0F1FDH                  ; enable DOS memory on page 0 handler
IF200   EQU     0F200H
CF218   EQU     0F218H                  ; PUT_P0
CF224   EQU     0F224H                  ; PUT_P2
DF23C   EQU     0F23CH                  ; default drive
H.PROM  EQU     0F24FH
DF2B8   EQU     0F2B8H                  ; number of VBLANK interrupts per 1/10 second
DF2BA   EQU     0F2BAH                  ; 'random' number
DF2C5   EQU     0F2C5H                  ; memory mapper table
DF2C7   EQU     0F2C7H                  ; current segment page 0
DF2C8   EQU     0F2C8H                  ; current segment page 1
DF2C9   EQU     0F2C9H                  ; current segment page 2
DF2CA   EQU     0F2CAH                  ; current segment page 3
DF2CB   EQU     0F2CBH                  ; segment page 0 at BDOS entry
DF2CC   EQU     0F2CCH                  ; segment page 1 at BDOS entry
DF2CD   EQU     0F2CDH                  ; segment page 2 at BDOS entry
DF2CF   EQU     0F2CFH                  ; BDOS data segment
DF2D0   EQU     0F2D0H                  ; BDOS code segment
DF2D1   EQU     0F2D1H                  ; temporary save stackpointer (KEYINT)
DF2D3   EQU     0F2D3H                  ; pointer to temporary stack (KEYINT)
DF2DC   EQU     0F2DCH                  ; pointer to prompt hander
DF2DE   EQU     0F2DEH                  ; pointer to BDOS handler
DF2E0   EQU     0F2E0H
DF2E1   EQU     0F2E1H
DF2E6   EQU     0F2E6H                  ; temporary save IX register BDOS call
DF2EA   EQU     0F2EAH
DF2EB   EQU     0F2EBH
DF2FE   EQU     0F2FEH                  ; pointer to temporary stack (BDOS)
DF30F   EQU     0F30FH                  ; double byte header table
DF314   EQU     0F314H                  ; TPA segment table
CF327   EQU     0F327H
CF32C   EQU     0F32CH
RAMAD0  EQU     0F341H
RAMAD1  EQU     0F342H
...
J410F:  DI
        ...
        CALL    CF1FD                   ; enable DOS memory on page 0
        LD      HL,I4100
        LD      DE,0
        LD      BC,04000H-0100H
        LD      A,2
...
J42C8:  LD      C,(HL)
        ...
        POP     HL
        JR      J42C8
J42DE:  LD      A,(DF2D0)               ; BDOS code segment
        LD      HL,01EEH
        ADD     HL,DE
        LD      (HL),A
        LD      A,(RAMAD3)
...
J495A:  LD      (DE),A
        ...
        DEC     A
        LD      (DF2CF),A               ; last segment for BDOS data
        DEC     A
        LD      (DF2D0),A               ; second last segment for BDOS code
...
J4B4C:  POP     DE
        ...
        LD      HL,IF200                ; memory mapper jump table
        PUSH    AF
        PUSH    DE
        RET
