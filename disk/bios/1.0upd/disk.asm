...
; Changes made:
; 1) Bugfixes of version 1.1 of diskrom
; 2) Own changes:
; - routine at 05A11H simpified.
; - routine at 05E52H simpified, does not support RAM other than 64 Kb
; - routine at 06306H simpified.
; extra room used to:
; - bugfix memory switching routines (CALSLT/CALLF/ENASLT)
; - boot from any drive at system startup
; - boot from default drive with _SYSTEM
...
YC000	equ	0C000H			; bootsector transferadres
...
YF23D	equ	0F23DH			; transferadres
...
YF2E2	equ	0F2E2H			; transferadres for recordoperation
...
M.4173	equ	04173H		; return to after statement
DECSUB	equ	0268CH		; dbl subtract
DECADD	equ	0269AH		; dbl add
DECDIV	equ	0289FH		; dbl divide
VMOVE	equ	02EF3H		; copy variable
VMOVFM	equ	02F08H		; copy variable
M.2F10	equ	02F10H		; copy variable
M.2F99	equ	02F99H		; integer to DAC
M.3042	equ	03042H		; convert sgn to dbl
M.30D1	equ	030D1H		; dbl to integer
M.325C	equ	0325CH		; sgn multiply
ACLOSE	equ	06C1CH		; close all channels
EXECLP	equ	04601H		; execution loop
BADRES	equ	06F0BH		; evaluate adres operand
GETVAR	equ	05EA4H		; get variable pointer
GETYPR	equ	05597H		; get operand type
FRETMP	equ	067D0H		; free temporary string
EVFSPC	equ	06A0EH		; evaluate filespec
EVADR	equ	0542FH		; evaluate adres operand
CNVADR	equ	05432H		; convert to adres
EVWORD	equ	04756H		; evaluate word operand
NEVBYT	equ	0521BH		; evaluate next byte operand
EVBYTE	equ	0521CH		; evaluate byte operand
CNVBYT	equ	0521FH		; convert to byte
OUTNL	equ	07328H		; newline to OUTDO
PTOLN	equ	054F7H		; convert pointers to linenumbers
CLOCHN	equ	06B24H		; close channel
MKPTRS	equ	04253H		; recalc linepointers
OPNCHN	equ	06AFAH		; open channel
SCROUT	equ	04AFFH		; output back to screen
EVEXPR	equ	04C64H		; evaluate expression
GTCHNP	equ	06A6DH		; get channel pointer
EVLEXP	equ	04C5FH		; evaluate =expression
ALSSPC	equ	0668EH		; allocate stringspace
ALTSTR	equ	06627H		; allocate temp string
CNVDAC	equ	0517AH		; convert DAC to other type
CSNG	equ	046FFH		; convert to SNG
COUTNL	equ	07323H		; newline to OUTDO if not at start of line
ERROR	equ	0406FH		; error
CHRGTR	equ	04666H		; CHRGTR
EXEBAS	equ	0409BH		; restart BASIC
QUITLD	equ	0739AH		; quit loading & start (headloop/executing)
SKPCHK	equ	06F1DH		; skip strong cassette devicecheck
M.6E41	equ	06E41H		; resume character putback routine

BASSCR	equ	07D2FH		; adres init screen for BASIC
SKPROM	equ	07D17H		; start MSX-BASIC
RSTCLR	equ	07E14H		; start MSX-BASIC program in ROM
TXTINI	equ	07D31H		; BASIC initscreen (without INITXT & CNSDFG)

CALBSV	equ	06E92H		; start of BSAVE routine
CALBLD	equ	06EC6H		; start of BLOAD routine
EXEBLD	equ	06EF4H		; finish BLOAD

RETRTN	equ	0F38BH		; adres with a RET instruction, allways available
...
;	     Inputs  -
;	     Outputs HL = lowest adres used by the base MSX-DOS system
...
A4331:	srl	c
        ...
        ld	a,(YF246)
        cp	(ix+000H)		; same driveid as dirsector buffer owner ?
...
A4367:	call	A4743			; update directory of disk when needed
        scf
        ret
...
A4376:	ld	a,0E5H
        ...
        or	l
        call	nz,A4F9B		; yep, release chain
        ...
        jp	A45CF			; write FAT (SHOULD BE: JP A45C4, write FAT when needed)
...
A43BB:	ld	a,(hl)
        ...
        ld	a,(iy+000H)		; yep, use the char of the orginal filename
A43C3:	ld	(de),a
        inc	hl
        inc	de
        inc	iy
...
A4403:	call	A4743			; update directory of disk when needed
        xor	a			; no error
        ret
;
A4408:	call	A4743			; update directory of disk when needed
...
A456F:	push	de
        ...
        call	z,A472D			; yep, write datasector buffer when needed
        call	A4555			; get pointer to DPB of current drive
        ld	a,(iy+019H)		; direntrynumber
...
;	  Subroutine write FAT when needed
;	     Inputs
;	     Outputs ________________________
...
A45FA:	ld	a,(ix+00AH)		; number of FATs
        ld	l,(ix+013H)
...
A4651:	jr	nz,A4647		; device or file with special fileattribute,
        ...
        jr	z,A4669			; nop,
        call	A4F9B			; release chain
        call	A45CF			; write FAT
A4669:	push	iy
        pop	de
        ld	hl,YF2B9
        ld	bc,11
...
A46A4:	push	bc
        call	A4743			; update directory of disk when needed
        pop	bc
        ld	b,(ix+000H)		; driveid
...
;	  Subroutine write datasector buffer when needed
;	     Inputs
;	     Outputs ________________________
...
;	  Subroutine write dirsector buffer when needed
;	     Inputs
;	     Outputs ________________________
...
;	  Subroutine BDOS 27 (random block read)
;	     Inputs
;	     Outputs ________________________
...
;	  Subroutine BDOS 26 (random block write)
;	     Inputs
;	     Outputs ________________________
...
A482D:	push	hl
        ...
        jr	nz,A482D
        pop	hl
        ld	(YF23D),hl		; restore transferadres
        ret
;
...
A487D:	ld	(YF2E8),hl		; number of records requested
        ...
        jr	nz,A48A8
        ld	a,(iy+00EH)
        ld	d,(iy+00FH)		; yep, use user set recordsize
        ld	e,a
        or	d			; zero recordsize ?
        jr	nz,A48A8
        ld	e,080H
        ld	(iy+00EH),e		; yep, use 128 bytes default
A48A8:	inc	d
        dec	d
        jr	nz,A48B1
        ld	a,e
        ...
        jr	c,A48B5
A48B1:	xor	a
        ld	(YF2E4+3),a		; make sure it's a valid record
A48B5:	ld	hl,(YF23D)
        ld	(YF2E2),hl		; current transferadres
        xor	a
...
A4989:	call	XF282
        ld	l,(iy+01CH)
        ld	h,(iy+01DH)		; current cluster of file
        ld	e,(iy+01EH)
        ld	d,(iy+01FH)		; current relative cluster of file
        ld	a,l
        or	h
        ...
        ld	l,(iy+01AH)
        ld	h,(iy+01BH)		; start cluster of file
        push	af
A49B0:	pop	af
A49B1:	call	XF285
        ld	a,b
...
A49F0:	push	de
        push	ix
        call	A472D			; write datasector buffer when needed
        pop	ix
        pop	de
        ld	a,(YF2E0)
...
A4A1B:	ld	a,1
        ...
        ld	bc,(YF2F8)
        add	hl,bc
        ld	(YF2E2),hl		; update current transferadres
        ld	hl,(YF34F)		; datasector buffer
        ld	de,(YF2F2)
        add	hl,de
        pop	de
...
A4AB1:	cp	00AH
        ...
        jr	nz,A4AA8
A4AB9:	ld	(YF300),hl
A4ABC:	ld	(YF2E2),de		; update current transferadres
        jp	nz,A4BE2
        res	6,(iy+018H)
        jp	A4BE2
;
...
A4AD7:	ld	de,(YF2E2)		; current transferadres
        inc	a
...
A4BE2:	call	XF291
        ld	hl,(YF2E2)		; current transferadres (end)
        ld	de,(YF23D)		; transferadres (begin)
        or	a
        sbc	hl,de
        ld	c,l
        ...
        ld	e,(iy+00EH)
        ld	d,(iy+00FH)		; user recordsize for Random Block
...
A4C22:	call	XF294
        ld	hl,(YF2EC)
        ld	(iy+01CH),l
        ld	(iy+01DH),h		; current cluster of file FCB
        ld	hl,(YF2EA)
        ld	(iy+01EH),l
        ld	(iy+01FH),h		; current relative cluster of file FCB
A4C37:	ld	hl,(YF2E4+0)
...
A4C47:	ld	hl,(YF23D)		; transferadres
        or	040H
        inc	a
...
A4CA3:	call	A487D			; initialize record info
        ...
        res	6,(iy+018H)		; flag FCB changed
        push	bc
...
A4D8C:	call	XF29D
        ...
        ld	hl,(YF2E2)		; current transferadres
        ld	de,(YF23D)		; transferadres
        or	a
        sbc	hl,de
        ld	de,(YF2F4+0)
        add	hl,de
...
A4E32:	ld	l,(iy+01AH)
        ...
        xor	a
        ld	(iy+01AH),a
        ld	(iy+01BH),a		; file has no start cluster (empty file)
        call	A4F9B			; release chain
        jr	A4E2C
;
A4E48:	call	XF2A0
...
;	  Subroutine get decoded characterpair (not needed)
;	     Inputs
;	     Outputs ________________________
...
A4EF8:	push	de
        pop	iy
        ld	c,(iy+020H)		; CR (current record)
        ld	b,(iy+00CH)		; EX (extent)
        ld	e,(iy+00EH)		; S2
        ld	d,000H
        sla	c
        srl	e
...
A4F5E:	push	hl
        ...
        jr	nz,A4F94
        ld	(iy+01AH),l
        ld	(iy+01BH),h		; start cluster of file
A4F94:	ex	de,hl
        pop	bc
        ld	(hl),c
        inc	hl
...
A4FC4:	ld	(YF30B),a
        ...
        ld	(hl),0E5H
A4FDC:	ld	bc,00020H
        call	XF1D9			; transfer direntry to DOS memory
        call	A4439			; get max record and extent
        ld	a,(YF30C)
        cp	b			; orginal FCB EX byte same as max extent ?
...
A4FEF:	ld	hl,(YF23D)		; transferadres
        ld	de,0000CH
        add	hl,de
...
A5058:	ld	(YF23D),de		; set transferadres
        ret
...
A509F:	ld	hl,00080H
        ld	(YF23D),hl		; default transferadres
        xor	a
...
A50B2:	ld	e,(hl)
        ...
        push	de
        pop	ix
        call	A45C4			; write FAT buffer when needed
        pop	af
        pop	hl
        dec	a
...
;	     Inputs  HL = adres of FCB+1,DE = destination
;	     Outputs ________________________
...
;
;	defb	"����������������������������������������������������������"
T5696:	defb	"��EA�A��EEEIII�����O�OUUY�������AIOU����������������������"
...
A57D6:	ld	(hl),0C9H
        ...
        ldir
        call	A402D
        ld	(H.RUNC+1),a		; init RUNC hook, to catch intialize interpeter
...
A58F0:	ld	(hl),0C3H
        ...
        inc	hl
        inc	hl
        djnz	A58F0			; initialize diskvars
        ld	hl,(AUTLIN)
        push	hl
        call	A5EC8
        ...
        xor	a
A5916:	add	a,(hl)
        jp	c,A5ECC			; invalid, error
        inc	hl
        inc	hl
        djnz	A5916
        cp	009H			; more as 8 drives ?
...
A592C:	ld	e,(hl)
        ...
        push	de
        pop	ix
        ld	(ix+000H),c		; set drivenumber in DPB
        inc	c
        push	bc
        ld	c,(ix+002H)
        ld	b,(ix+003H)		; sectorsize
        ...
        call	A4916			; * FAT size
        inc	bc			; and a FAT flag
        ld	l,c
        ld	h,b
        call	A5EC8			; allocate FAT buffer
        ld	(YF349),hl		; base system bottom sofar
        ld	(hl),0FFH		; flag invalid FAT buffer
        inc	hl
        ld	(ix+013H),l
        ld	(ix+014H),h		; pointer to FAT buffer
        pop	bc
        pop	hl
...
A5967:	inc	hl
        ...
        pop	ix
        ld	iy,(EXPTBL-1+0)
        call	CALSLT			; setup startup screen
...
A59CB:	ld	sp,0C200H		; stack just above temp startbuffer
        ld	a,(H.STKE+0)
...
;
A59E9:	ld	ix,RSTCLR		; start BASIC program in ROM
A59ED:	call	A5C16			; initialize diskbasic
        jp	CALBAS
;
A59F3:	ld	hl,A5B3A
        push	hl			; if quit anywhere start diskbasic
        call	ABOOT			; NEW: READ BOOTSECTOR FROM ANY DRIVE
...
;
; CHANGED TO CREATE ROOM FOR NEW CODE
; Init procedure simplified, but code is compatible
;
A5A11:
...
; start of updated code
        ...
        ld	a,(YF247)		; NEW: _SYSTEM FROM DEFAULT DRIVE
        call	A609A			; mark FAT buffer of default drive as invalid
        ld	hl,(YF349)
        ld	(YF34B),hl		; set MSXDOS system bottom to base system bottom
        call	A5AE7			; read bootsector from the default drive
        jp	c,A5B3A			; error reading bootsector or bootsector non-bootable
        ld	(YF346),a		; bootable disk, flag CALL SYSTEM is valid
        ld	a,(RAMAD0)
        ld	h,000H
        call	A649C			; enable ram on page 0 (649C works because RAM is in one slot!)
        xor	a
        ld	l,a
        ld	h,a
A5A36:	ld	(hl),a
        inc	l
        jr	nz,A5A36		; clear 0000H-00FFH
        ld	hl,A655C
        ld	de,X003B
        ld	bc,0001AH
        ldir				; copy slotswitching code at 003BH
        ld	bc,LDOSR		; total count
        call	A5EAD			; allocate MSXDOS memory
        push	hl
        ex	de,hl
        ld	hl,A6336		; routines in ROM
        push	hl
        ldir				; copy interrupt/slotswitching routines
        pop	bc
        pop	hl
        push	hl
        or	a
        sbc	hl,bc
        ld	de,T632E		; relocate info
        call	A6306			; relocate code
        ld	hl,T5B0F		; jumptable
A5A84:	ld	e,(hl)
        inc	hl
        ld	d,(hl)			; adres jumpentry
        inc	hl
        ld	a,e
        or	d
        jr	z,A5A98			; end of table
        ld	c,(hl)
        inc	hl
        ld	b,0			; offset
        ex	(sp),hl			; previous routine adres
        add	hl,bc
        ex	de,hl
        ld	(hl),0C3h
        inc	hl
        ...
        inc	hl
        ld	(hl),d
        ex	de,hl			; make jumpentry
        ex	(sp),hl
        jr	A5A84			; next entry
A5A98:	pop	hl
        ld	bc,000A0H		; temporary stackspace for KEYINT
        call	A5EAD			; allocate MSXDOS memory
        scf				; start bootcode with Cx set
A5ADB:	ld	hl,YF323		; adres diskerror handler pointer
        ld	de,XF368
        ld	a,(YF340)		; cold boot flag
        jp	YC000+01EH		; start bootsector code
...
T5B0F:	defw	KEYINT
        ...
        defw	0
;
; CHANGED: READ BOOTSECTOR FROM DEFAULT DRIVE
;
A5AE7:	ld	a,(YF247)
        or	a
        push	af
        ...
        ret
;
; ROOM CREATED BY CHANGE IS FILLED WITH NOPS
; TO KEEP FOLLOWING CODE AT SAME ADDRESSES
;
        defs	05B1AH-05A11H-($-A5A11),0
;
; END OF CHANGE
;
...
A5B3A:	call	A5C60			; disable XFER,ENARAM,ENAKRN routines
        ...
        jr	nz,A5B6E		; this was a warm boot, no need to start autoexec.bas
        ld	(YF346),a		; this was a cold boot, flag CALL SYSTEM invalid
        ld	hl,T5B38
...
A5B6E:	ld	a,(WBOOT)
        cp	0C3H
        jr	nz,A5B9C		; diskbasic not started from MSXDOS, just start diskbasic
        ld	hl,00080H
        ld	b,(hl)
        inc	b
        dec	b
        jr	z,A5B9C			; no parameter specified (after the BASIC command), just start diskbasic
A5B7D:	inc	hl
        ld	a,(hl)
        cp	" "
        jr	nz,A5B87
...
A5BA0:	ld	sp,0C200H
        ...
        ld	hl,0FFFFH
        ld	(CURLIN),hl		; interpeter in direct mode
        ...
        ld	ix,TXTINI
        call	CALBAS			; setup basic startup screen
        call	A5F86
        defb	13,10,"Disk BASIC version 1.0",13,10,0
        ld	hl,M.4173
        push	hl			; after the RUN command completes
        ld	hl,BUF+9
        push	hl
        ld	hl,BUF+64
        push	hl
...
A5C16:	ld	hl,T72AE
        ...
        ld	a,(YF345)
        ld	c,a
        ld	b,000H			; number of FCB�s
        ld	de,37
        call	A4916			; * 37
        call	A5EB8			; allocate basic memory
        ld	(YF353),hl
        ld	bc,25
        call	A5EB8			; allocate basic memory
        ld	(XF377+1),hl
...
;	     Inputs  A = driveid, B = number of sectors, C = mediadescriptor, DE = start sector, HL = transferadres
;	     Outputs -
...
;
        ...
        ret
;
; CHANGED TO CREATE ROOM FOR FUTURE EXPANDING CODE
; routine functions have compatible code
;
A6306:	ld	a,(de)
        inc	de
        or	a
...
R0000:	ld	hl,(D635E+1)
        ld	a,l
        or	h			; already in KEYINT ?
        pop	hl
        ld	ix,KEYINT
        ld	iy,(EXPTBL-1+0)
        jr	nz,A6375		; yep, start BIOS KEYINT
        pop	af
R0001:	ld	(D635E+1),sp		; save current stackpointer
R0002:	ld	sp,A6336		; switch to special KEYINT stack (because stackpointer in DOS could be in page 0)
        call	CALSLT			; start BIOS KEYINT
        di
D635E:	ld	sp,00000H		; restore orginal stackpointer
        push	hl
        ld	hl,0
R0003:	ld	(D635E+1),hl		; flag not in KEYINT
        pop	hl
A6369:	ex	af,af'
        exx
        pop	af
...
A63F4:
R0100:	call	A64C2			; calculate masks
R0101:	jp	m,A6404			; expanded slot specified,
        in	a,(0A8H)
        ld	d,a			; save primary slotregister
        and	c
        or	b			; change slot of specified page
        call	RDPRIM			; change primary slotregister, read memory and restore primary slotregister
        ld	a,e
        ret
;
A6404:	push	hl
R0102:	call	A64E7			; setup secundairy slotregister
        ex	(sp),hl
        push	bc
R0103:	call	A63F4			; RDSLT, but now only with primary slot
        jr	A6436			; restore secundairy slotregister

; DOS WRSLT routine
;
A6415:	push	de
R0200:	call	A64C2			; calculate masks
R0201:	jp	m,A6425			; expanded slot specified,
        pop	de
        in	a,(0A8H)
        ld	d,a			; save primary slotregister
        and	c
        or	b			; change slot of specified page
        jp	WRPRIM			; change primairy slotregister, write memory and restore primary slotregister
;
A6425:	ex	(sp),hl
        push	hl
R0202:	call	A64E7			; setup secundairy slotregister
        pop	de
        ex	(sp),hl
        push	bc
R0203:	call	A6415			; WRSLT, but now only with primary slot
A6436:	pop	bc
        ex	(sp),hl
        push	af
        ld	a,b
        and	03FH
        or	c
        cp	b			; restore secundairy slotregister currently in page 3 ?
        jr	nz,ARDWR		; nope, use helper routine in page 0
        ld	a,l
        ld	(0FFFFH),a		; yep, restore secundairy slotregister directly to avoid crash for page 0 addresses
ARDWR:	call	nz,X0046		; restore secundairy slotregister
        pop	af
        pop	hl
        ret
...
A6455:	exx
        ...
        push	ix
        pop	hl
R0300:	call	A64C2			; calculate masks
R0301:	jp	m,A646C			; expanded slot specified,
        in	a,(0A8H)
        push	af			; save primary slotregister
        and	c
        or	b			; change slot of specified page
        exx
        jp	CLPRIM			; change primary slotregister, execute routine and restore primary slotregister
;
A646C:
R0302:	call	A64E7			; setup secundairy slotregister
        push	af
        pop	iy
        push	hl
        push	bc
        ...
        ld	hl,SLTTBL
        add	hl,bc
        ld	(hl),a			; update SLTTBL to reflect the change of the secundairy slotregister
        push	hl
        ex	af,af'
        exx
R0303:	call	A6455			; CALSLT, but now only with primary slot
        exx
        ex	af,af'
        pop	hl
        pop	bc
        ...
        or	c
        di
        cp	b			; same primairy slot ?
        jr	nz,ACALL		; nope, use the helper routine in page 0
        ld	a,e
        ld	(0FFFFH),a		; yep, setup secudairy slotregister directly to avoid crash for page 0 addresses
ACALL:	call	nz,X004B		; nope, setup secundairy slotregister
        ld	(hl),e
        ex	af,af'
        exx
        ret
...
;
A649C:
R0400:	call	A64C2			; calculate masks
R0401:	jp	m,A64A9			; expanded slot specified,
A64A2:	in	a,(0A8H)
        and	c
        or	b
        out	(0A8H),a		; change slot of specified page
        ret
;
A64A9:
A64AF:	push	hl
R0402:	call	A64E7			; setup secundairy slotregister
        ld	c,a
        ld	b,000H
        ld	a,l
        and	h
        ...
        pop	hl
        ld	a,c
        jr	A649C			; ENASLT, but now only with primary slot
;
A64C2:	di
        push	af
        and	003H
...
A64DD:	add	a,055H
        djnz	A64DD
        ld	d,a			; primary slot mask (PP PP PP PP)
        ld	a,h
        rlca
        rlca
        and	003H
...
A64CD:	rlca
        rlca
        djnz	A64CD
        ld	e,a			; select page mask
        cpl
        ld	c,a			; clear page mask
        ld	a,d
        and	e
        ld	b,a			; set primary slot mask
        pop	af
        and	a			; sign flag set if expanded slot
        ret

A64E7:	push	af
        and	00CH
        rrca
        rrca				; requested secundairy slot in b1-b0
        ld	b,a
        inc	b
        ld	a,0ABH
A64FF:	add	a,055H
        ...
        ld	h,a
        pop	af
        and	003H			; requested primary slot in b1-b0
        push	af
        rrca
        rrca
        ld	c,a			; OR mask for selecting primary slot in page 3
        in	a,(0A8H)
        ld	b,a			; current primary slotregister
        and	03FH
        or	c
        cp	b			; change secundairy slotregister currently in page 3 ?
        jr	nz,ASSEC		; no, use helper routine in page 0
        ld	a,(0FFFFH)
        cpl
        ld	l,a
        and	h
        or	d
        ld	(0FFFFH),a		; yep adjust secundairy slotregister directly to avoid crash for page 0 addresses
        pop	af
        ret
ASSEC:	call	X003B			; save and change secundairy slotregister
        pop	af
        ret
...
;
A63A1:	push	af
        ld	a,(YF348)		; slotid kernel ROM
        jr	A63AD

; DOS ENARAM routine
;
A63A3:	push	af
        ld	a,(RAMAD1)		; slotid TPA (RAM) page 1
A63AD:	push	hl
        push	de
        push	bc
        ld	h,040H
...
A637B:	push	af
        ...
        push	bc
        call	A402D
        push	af			; save slotid currently on page 1 (the diskdriver ROM)
        ld	h,040H
        ld	a,(RAMAD1)
        call	ENASLT			; enable TPA (RAM) on page 1
        pop	af
        pop	bc
        pop	de
        pop	hl
        ldir				; transfer to/from TPA
        push	hl
        push	de
        push	bc
        call	XF368			; enable kernel ROM on page 1
        ld	h,040H
        call	ENASLT			; restore diskdriver ROM on page 1
        pop	bc
        pop	de
        pop	hl
        pop	af
...
; change secundairy slotregister (copied to 003BH)
...
; restore secundairy slotregister (copied to 0046H)

        out	(0A8H),a
        ld	a,l
        jr	A656F

; setup secundairy slotregister (copied to 004BH)
...
A656F:	ld	(YFFFF),a
        ...
        ret
;
; NEW ROUTINE: READ BOOTSECTOR FROM ANY PHYSICAL DRIVE
;
ABOOT:	ld	hl,AINCP
        ld	de,BUF+10
        ld	bc,LINCP
        ldir
        ld	a,(YF347)
        ld	b,a			; try all drives
        ld	c,0			; begin with A:
ANXBT:	ld	a,c
        ld	(YF247),a		; set default drive
        call	ADPTH			; hook PROMPT
        push	bc
        call	A5AE7			; read bootsector from the default drive
        pop	bc
        sbc	a,a
        ld	e,a			; validflag
        ld	a,(XF24F)
        cp	0C3H			; used PROMPT (panthom drive)
        jr	z,ANPTH			; nop,
        call	ADPTH			; hook PROMPT
        ld	a,c
        dec	a
        ld	(YF247),a		; physical drive panthomed is now default
        push	bc
        call	A5AE7			; read bootsector from the default drive (just restoring panthom status)
        pop	bc
        ld	e,0FFH			; invalid
ANPTH:	call	AEPTH			; clear hook PROMPT
        xor	a
        sub	e
        ret	nc			; valid bootsector, quit
        inc	c
        djnz	ANXBT			; next drive
        xor	a
        ld	(YF247),a
        scf				; none found, restore A: as default drive
        ret
...
LINCP	equ	$-AINCP
;
; UNUSED SPACE IS FILLED WITH NOPS, TO KEEP FOLLOWING CODE AT
; THE SAME ADDRESSES FOR MAXIUM COMPATIBLITY
;
        defs	06576H-06306H-($-A6306),0
;
; END OF CHANGE
;
A6576:	ld	a,(H.GETP+0)
...
A65F7:	push	af
        ...
        inc	hl
        ld	a,(hl)
        cp	009H			; channel in use by a disk device ?
...
;
A6645:	ld	hl,BUF+84
        ld	(YF23D),hl		; transferadres in BUF
        ld	de,BUF+10
        xor	a
...
A66A4:	ld	ix,RETRTN
        ld	iy,00200H
        call	A6663			; take control from caller
        pop	hl
        ld	a,(hl)
        and	a
        ret
...
A66B3:	ei
        ...
        call	A72DA			; already end of statement ?
        ld	a,e
        ret	z			; yes, quit
        push	af
        push	hl
...
A66CD:	add	hl,sp
        ...
        ld	ix,EVWORD
        call	A731E			; evaluate word operand
        dec	de
        inc	d
...
;	  Subroutine OPEN statement  (H.NULO)
;	     Inputs
;	     Outputs ________________________
...
A67C1:	push	bc
        ...
        call	A67FE			; setup FCB fields
        ld	hl,(PTRFIL)
        ld	(hl),001H		; first in input mode
        ld	bc,00006H
        add	hl,bc
        ld	(hl),0FFH
        ld	hl,(PTRFIL)
A67DA:	push	hl
        ...
        ld	(hl),002H		; continue in output mode
        pop	hl
        ld	bc,00021H
...
A682A:	push	hl
        ...
        push	hl
        ld	(YF23D),hl		; transferadres is I/O channel buffer
        ex	de,hl
        inc	hl
        ld	e,(hl)
        inc	hl
...
;
;
T69B9:	defb	":",092H,000H,000H,000H
...
A717F:	ld	de,(NULBUF)
        ld	hl,00100H
A7186:	ld	(YF23D),de		; transferadres
        ret
;
A718B:	ld	de,(YF353)
...
;	     Inputs  HL = source adres, DE = destition adres, BC = size
;	     Outputs ________________________
