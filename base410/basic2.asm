...
; MSX BASIC ROM, MSX Turbo-R version (version 4.1)
...
; #####################
; MSX Turbo-R has a patch for the KANJI screenmodes
...
C4F2A:  CP      1BH
        ...
        ELSE
; Panasonic FS-A1GT use H.OKNO for MSX-MIDI
        NOP
        NOP
        NOP
...
C5439:  LD      BC,C2F8A
        ...
        ELSE
; Panasonic FS-A1GT use H.FRQI for MSX-MIDI
...
; #####################
; MSX 2 and above have a patch for CALSLT to page 2 and 3
...
; #####################
; MSX Turbo-R has a patch for KANJI screenmodes
...
;       CSAVE statement handler

; #####################
; MSX Turbo-R does not support CSAVE
...
;       BSAVE to cassette subroutine here

; #####################
; MSX Turbo-R does not support BSAVE to cassette
...
;       BLOAD from cassette subroutine

; #####################
; MSX 2 Turbo-R does not support BLOAD from cassette
...
;       CLOAD statement handler

; #####################
; MSX Turbo-R does not support CLOAD
...
J71B6:  CALL    C72CD                   ; bad filename error if random mode
        ...
        LD      (PTRFIL),HL             ; interpreter input/output device
        LD      (HL),E
J71C2:  RET
...
C71C7:  DEFW    J6E6B                   ; open, bad filename
        DEFW    C475A                   ; close, illegal function call
        DEFW    J6E86                   ; random, sequential i/o only error
        DEFW    C475A                   ; output, illegal function call
        DEFW    C475A                   ; input, illegal function call
        DEFW    C475A                   ; illegal function call
        DEFW    C475A                   ; illegal function call
        DEFW    C475A                   ; illegal function call
...
; #####################
; MSX Turbo-R does not support cassette
...
C79A9:  RET     NZ
        ...
        POP     HL
        AND     A
        JP      NZ,C475A                ; yep, illegal function call
J79BE:  XOR     A
        JP      CLS
; #####################

; #####################
;       Bugfix  call init,statement and device routines in page 2 or 3
...
C7B61:  LD      HL,0
        ...
        LD      A,L
        LD      (D.FFFF),A                      ; if primary slot is expanded, select secundairy slot RAM in page 2 and 3
...
J7B80:  RR      C
        ...
        IN      A,(0A8H)
        LD      C,A                             ; save current primairy slotregister
        XOR     A
        OUT     (0A8H),A                        ; primairy slot 0 in all pages
        LD      A,(D.FFFF)
        CPL
        LD      L,A                             ; current secundiary slotregister slot 0
        LD      A,40H
        OUT     (0A8H),A                        ; primairy slot 1 in page 3, primairy slot 0 in other pages
        LD      A,(D.FFFF)
        CPL
        LD      H,A                             ; current secundiary slotregister slot 1
        LD      A,80H
        OUT     (0A8H),A                        ; primairy slot 2 in page 3, primairy slot 0 in other pages
        LD      A,(D.FFFF)
        CPL
        LD      E,A                             ; current secundiary slotregister slot 2
        LD      A,0C0H
        OUT     (0A8H),A                        ; primairy slot 3 in page 3, primairy slot 0 in other pages
        LD      A,(D.FFFF)
        CPL
        LD      D,A                             ; current secundiary slotregister slot 3
        LD      A,C
        OUT     (0A8H),A                        ; restore primairy slotregister
        LD      (SLTTBL+0),HL
        EX      DE,HL
        LD      (SLTTBL+2),HL                   ; save secundiary slotregisters
...
; Panasonic FS-A1GT has MSX-MIDI build in
...
; #####################
;       Bugfix  call init,statement and device routines in page 2 or 3, part 2
...
J7D87:  CALL    C7E1A
        ...
        PUSH    AF
        POP     IY
        CALL    NZ,C7FF2                ; if extension ROM has INIT, call INIT after VR
        POP     BC
        POP     HL
