...
; MSX SUBROM, MSX Turbo-R version (v4.1)
...
D$F33B  EQU     0F33BH  ; address pathnamebuffer, dos kernel 2.x
RDPRIM  EQU     0F380H
WRPRIM  EQU     0F385H
CLPRIM  EQU     0F38CH
CLPRM1  EQU     0F398H
...
C076A:  CALL    C0776                   ; clear spritepatterntable, initialize planes and colortable
        PUSH    HL
        LD      HL,0
        CALL    C0971                   ; setup vdp addressregister for active screenpage (write)
        POP     HL
        RET
...
;       Inputs          HL=VRAM base address, DE=RAM address, BC=size
;       Outputs         ________________________

C0829:  CALL    C097F                   ; setup vdp addressregister for active screenpage (read)
        LD      A,C
...
?0956:  CALL    C096A                   ; VRAM address in 16 Kb range

        ENDIF

;       Subroutine      S.WRTVRM
;       Inputs          HL=VRAM base address
;       Outputs         ________________________
...
C0961:  CALL    C096A                   ; VRAM address in 16 Kb range

;       Subroutine      S.RDVRM
;       Inputs          HL=VRAM base address
;       Outputs         ________________________

C0964:  CALL    C097F                   ; setup vdp addressregister for active screenpage (read)
        IN      A,(98H)
        RET
...
C0971:  PUSH    BC
        ...
        PUSH    HL
        EX      DE,HL
        CALL    C09A2                   ; convert page base address to VRAM address
        LD      A,H
        AND     3FH
        OR      40H
        JR      J0989
...
C09A2:  LD      A,(SCRMOD)
        ...
        EX      DE,HL
        LD      D,A                     ; VRAM address=base address
        RET
...
;       Inputs          HL=VRAM base address, BC=size, A=fillbyte
;       Outputs         ________________________
...
;       Subroutine      fill VRAM
;       Inputs          HL=VRAM address, BC=size, A=fillbyte
;       Outputs         ________________________
...
J0BF1:  LD      C,10H                   ; M2=1, M1=0 for both modes
        ...
        LD      HL,(TXTCGP)
        LD      B,00H
        CALL    C0D87                   ; setup VDP patterngenerator address
        ADD     A,A
        LD      HL,(TXTNAM)
        CALL    C0D7B                   ; setup VDP patternnametable address
...
J0C13:  LD      A,(DPPAGE)
        ...
        LD      D,A
        LD      B,07H
        CALL    C0D95                   ; setup VDP colortable address
        JP      J0733                   ; setup VDP registers and digitizehardware
...
C0C3D:  CALL    C0DCE                   ; set VDP in 192 lines mode
        ...
        CALL    C0D87                   ; setup VDP patterngenerator address
        LD      HL,(T32PAT)
        CALL    C0DC1                   ; setup VDP spritepattern address
        LD      HL,(T32NAM)
        ADD     A,A
        CALL    C0D7B                   ; setup VDP patternnametable address
        LD      E,B
...
J0C81:  CALL    C0DDE                   ; set VDP mode bits
        ...
        LD      HL,(ATRBAS)
J0CBD:  CALL    C0DAC                   ; setup VDP spriteattribute address
        JP      J0733                   ; setup VDP registers and digitizehardware
...
J0D00:  LD      A,(DPPAGE)
        ...
        ADD     A,A
        ADD     A,A
        CALL    C0DBE                   ; setup VDP spritepattern address with PATBAS
        LD      HL,(NAMBAS)
        ADD     A,A
        LD      B,1FH
        CALL    C0D7B                   ; setup VDP patternnametable address
...
J0D17:  LD      E,00H
        LD      B,03H
        CALL    C0DA9                   ; setup VDP spriteattribute address with ATRBAS
        JP      J0733                   ; setup VDP registers and digitizehardware
...
C0F56:  XOR     A
        ...
        PUSH    AF
        LD      H,1
        CALL    C0FDC                   ; convert screenlocation to VRAM address
        POP     AF
        LD      A,(LINLEN)
        JR      NZ,J0F66
        SUB     40
...
J0FBD:  PUSH    HL
        CALL    C0FDC                   ; convert screenlocation to VRAM base address
        CALL    C0971                   ; setup vdp addressregister for active screenpage (write)
        LD      A,C
        OUT     (98H),A
        POP     HL
...
;       Inputs          HL=vram base address, DE=ram address, B=size
;       Outputs         ________________________

C0FC9:  CALL    C097F                   ; setup vdp addressregister for active screenpage (read)
        EX      DE,HL
        LD      C,98H
        INIR
        RET
...
;       Inputs          HL=ram address, DE=vram base address, B=size
;       Outputs         ________________________
...
C1011:  PUSH    HL
        LD      D,A
        CALL    C10A0                   ; get palettetable address in vram
        CALL    C097F                   ; setup vdp addressregister for active screenpage (read)
        EX      (SP),HL
        EX      (SP),HL
        IN      A,(98H)
        LD      B,A
...
J1025:  PUSH    HL
        ...
        CALL    C10A0                   ; get palletetable address in vram
        CALL    C097F                   ; setup vdp addressregister for active screenpage (read)
        LD      B,D
        LD      C,16
...
J1137:  LD      C,0D8H                  ; io address kanji-1
J1139:  OUT     (C),L
        ADD     HL,HL
...
J17EB:  POP     BC
        ...
        LD      HL,(DSCTMP+1)           ; address of stringspace
        POP     DE
        PUSH    HL
        LD      A,(DE)
        RLCA
...
;       Inputs          ________________________
;       Outputs         A=size,DE=address of string
...
J1CDC:  CP      10H
        ...
        SUB     0F5H
        JR      NC,J1CF3                ; 011H - 01BH code, 0-10 constant
        CP      0FEH
...
J1CF8:  LD      L,A
        ...
        LD      HL,M46E6                ; basicpointer to special basictext to handle constant
        POP     AF
...
J235C:  CALL    C2818           ; evaluate address operand
        PUSH    DE
        RST     S.SYNCHR
        DEFB    ","             ; check for ,
        RST     S.GETBYT
...
J236F:  CALL    C27CA           ; convert DAC to address operand
        CALL    C0964           ; RDVRM
        PUSH    AF
        LD      HL,0
        CALL    C0964           ; RDVRM
...
;       Inputs          ________________________
;       Outputs         ________________________
;       Remark          Only called from 239D, which is unused: UNUSED CODE
...
;       Subroutine      convert DAC to address operand
;       Inputs          ________________________
;       Outputs         ________________________
...
I2C5A:	DEFB	000H                    ; R#0: DG=0 (Digitize off), IE1=0 (VR int off), M5=0, M4=0, M3=0 (screen 1)
        ...
        DEFB	008H                    ; R#8: TP=0 (color 0 is transparent), CBD=0 (colorbus not input), VRS1=0, VRS0=0 (64 KB RAM chip), SPD=0 (sprites enabled), B/W=0 (blank/white off)
        ...
        DEFB    000H                    ; R#24: not used on V9958
        DEFB	000H                    ; R#25: CMD=0 (command mode not in screen 0-4, V9938 compatibel), VDS=0, YAE=0, YJK=0, WTE=0 (no wait to CPU), MSK=0, SP2=0
        DEFB    000H                    ; R#26: H8=0, H7=0, H6=0, H5=0, H4=0, H3=0
        DEFB    000H                    ; R#27: H0=0, H1=0, H2=0
...
J2FEF:  PUSH    HL
        ...
        INC     HL
        INC     HL
        INC     HL                      ; FNPTR (address of the filename)
        CALL    C322E                   ; parse filespec and check if diskdrive
        POP     HL
        LD      E,1                     ; flag write
J2FFA:  PUSH    DE
        LD      C,(HL)
        INC     HL
        LD      B,(HL)                  ; starting address
        INC     HL
        LD      E,(HL)
        INC     HL
        LD      D,(HL)                  ; ending address
        EX      DE,HL
        AND     A
        SBC     HL,BC
...
C323C:  LD      HL,(BUF+4)              ; DPTR, address of array
        LD      C,(HL)
        INC     HL
        LD      B,(HL)                  ; NX
        INC     HL
...
J339F:  POP     AF
        ...
        LD      A,H
        OR      L
        JR      NZ,J33C0                ; size<>0, use the given startaddress
        LD      HL,0FE00H
...
J33C0:  LD      (BUF+19),DE             ; save DTA address
...
J3B48:  RRCA
        ...
        LD      A,E
        DI
        CALL    M51D7                   ; helper routine in basic rom, to switch subrom away
        EI
        EXX
        RET
