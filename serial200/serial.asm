...
Y6000	equ	06000H		; rs232 receive buffers
YA000	equ	Y6000+04000H
Y6400	equ	06400H		; rs232 flags
YA400	equ	Y6400+04000H
                                ; +0
                                ; +1 backup char
                                ; +2 backup char error status
                                ; +3
                                ; +4 timeout
                                ; +5 chars in receiverbuffer
                                ; +6 getposition
                                ; +7
                                ; +8 error flags
                                ; +9 flags
...
Y6600	equ	06600H		; rs232 new style table
Y661E	equ	0661EH		; channel slotmask table (entries are 3 bytes)
Y664B	equ	0664BH		; channel mode table
Y665A	equ	0665AH		; channel file control block pointer table
Y6676	equ	06676H		; flag
Y6678	equ	06678H		; saved stackpointer
Y667A	equ	0667AH		; saved basicpointer
Y667C	equ	0667CH		; temporary save
Y667E	equ	0667EH		; save
...
Y6689	equ	06689H		; flags current selected channel
Y668B	equ	0668BH		; memorymapped adres current selected channel
Y668D	equ	0668DH		; receivebuffer current selected channel
Y668F	equ	0668FH		; number of channels in this interface
Y6690	equ	06690H		; total number of channels new style rs232
Y6691	equ	06691H		; channel number first new style rs232
Y6692	equ	06692H		; first trap number new style rs232
Y6693	equ	06693H		; enable Rx int
Y6694	equ	06694H		; disable Rx int
Y6695	equ	06695H		; enable/disable for first channel
Y6697	equ	06697H		; enable/disable for other channels
Y6699	equ	06699H		; saved FCALL
Y669E	equ	0669EH		; saved H.NEWS
Y66A3	equ	066A3H		; saved H.KEYI
Y66A8	equ	066A8H		; COMTERM
Y66A9	equ	066A9H		; COMTERM
Y66AA	equ	066AAH		; COMTERM
Y66AB	equ	066ABH		; COMTERM
Y66AC	equ	066ACH		; COMTERM
Y66AD	equ	066ADH		; COMTERM
Y66AE	equ	066AEH		; COMINI
Y66BB	equ	066BBH		; COMTERM, saved F5,F6 and F7
Y66EB	equ	066EBH		; old style rs232 channelnumber (between new style)
...
;
A40B2:	xor	a
        ld	(Y6676),a	; clear flag
        dec	a
        ld	(Y66EB),a	; no old style channel between new style channels
        di
        call	A53A8		; slotid on page 2
        push	af
...
A4105:	xor	a
        ...
        jp	z,A41C1		; no, take control of rs232
        ld	hl,Y6700
        ld	a,(Y6683)
        ld	b,a		; my slotid
        ld	de,00800H
...
A413B:	ld	a,(hl)		; slotid of interface
        ...
        pop	bc
        ex	de,hl
        or	a		; new style rs232 ?
        jr	nz,A4154	; yes, leave control of rs232 where it is
        inc	hl
        inc	hl
        djnz	A413B		; next interface
        jp	A41E3		; all others are old rs232's, take control of rs232
;
A4154:	ld	a,0FFH
        ...
        dec	hl
        ld	a,(OLDINT+0)
        or	a		; has a old style rs232 hooked H.KEYI ?
        jr	z,A4187		; no,
        ...
        call	RDSLT
        pop	hl
        or	a		; flag already set ?
        jr	nz,A4187	; yes, skip
        ld	a,(hl)		; slotid of last 'new style' rs232 interface before me
        push	hl
        ld	hl,Y6691
...
A4201:	ld	(hl),a
        inc	hl
        djnz	A4201		; clear rs232 new style table
        ld	hl,Y664B
...
A4439:	sub	"0"
        ...
        ret	c			; not a new style channel, device not recognized
        inc	hl
        ld	a,(hl)
        ...
        ld	hl,Y6691
        sub	(hl)			; - first channel number new style rs232
        ld	hl,Y66EB
        cp	(hl)
        adc	a,0FFH			; adjust for old style between new style
...
A450A:	call	A4709		; set current channel from file control block
...
A456C:	call	A4709		; set current channel from file control block
        ld	a,c
        call	A56E5		; RS232.SNDCHR
        ei
        jp	c,A4871
...
A457B:	call	A4709		; set current channel from file control block
...
; Device LOC
;
A4611:	call	A4709		; set current channel from file control block
        push	hl
...
; Device LOF
;
A466C:	call	A4709		; set current channel from file control block
        push	hl
...
A4688:	call	A4709		; set current channel from file control block
        push	hl
        call	A4694
        jp	m,A4871
        jr	A4673
...
; Device Backup
;
A46B4:	call	A4709		; set current channel from file control block
;
; RS232.BACKUP
;
A46B7:	push	af
...
; set current channel from file control block
...
A4712:	inc	b
        ld	a,(hl)
        inc	hl
        ld	c,(hl)		; file control block associated with channel
        inc	hl
        cp	e
        jr	nz,A4712
        ld	a,c
        cp	d		; same as supplied file control block ?
        jr	nz,A4712	; nope, next
        ld	a,b
...
A474F:	push	bc
        ...
        push	hl
        ld	hl,Y6690
        add	a,(hl)		; number of new style channels
        pop	hl
        push	af
        jp	A4826
...
A4761:	push	bc
        ...
        push	af
        ld	a,(Y6691)
        ld	d,a		; first channel number new style rs232
        ld	e,0
        ld	hl,Y664B
        add	a,l
        ld	l,a
...
A4791:	inc	hl
        inc	d
        inc	e
        ld	a,(Y6690)	; number of new style channels
        cp	e
...
A4BA3:	ei
        ...
        call	A48E8
        defb	")"
A4BB9:	ld	(Y6685),hl	; save basicpointer
        xor	a
        ld	(Y66A8),a
        ld	(Y66AC),a
        ld	(Y66AA),a
...
T4C64:	ld	(Y66A8),a
        ...
        ld	bc,00030H
        ldir
        ld	hl,(Y6685)	; restore basicpointer
        ld	a,(Y66AD)
        or	a
        push	af
        call	z,A4DEB
...
;
A5001:	ei
A5002:	call	A53C7		; buid slotinfo per channel table
A5005:	call	A5161
        call	A51E6
        call	A48E8
        defb	","
...
A51AD:	ld	e,a
        ...
        call	A51C7		; RS232.SETCHN
        pop	bc
        ret	nc		; channel is new style, quit
A51BE:	pop	hl
        ld	hl,(Y6685)	; restore basicpointer
...
A51C7:	push	de
        ...
        pop	de
        ld	a,(Y66EB)
        cp	e		; old style channel (between new style channels) ?
        scf
        ret	z		; yep, quit
        ld	a,(Y6690)
        ld	c,a		; number of new style channels
        ld	a,(Y6691)
        ld	b,a		; first channel number new style rs232
        add	a,c
        ld	c,a
        ld	a,e
...
A53E9:	ld	a,(hl)
        ...
        inc	hl
        ld	a,(Y6690)
        ld	(Y66EB),a	; old style channel between new style channels
        inc	a
        ld	(Y6690),a	; +1
        jr	A53E9		; next entry
;
...
A5434:	dec	a
        ...
        ld	hl,Y6690
        ld	a,(hl)		; number of new style channels
...
; enable hardware current channel on page 2
...
A5448:	di
        ...
        ld	hl,(Y6697)	; other channel, no i/o space bit
A5456:	ld	(Y6693),hl
        ld	hl,Y661E
        ld	c,a
        add	a,a
